...
try:
    ...
    logging.warning("june-grpc-api package not available. Install it to run gRPC load tests.")
...
class GRPCLoadTest:
    ...
    async def run(self):
        ...
        if not GRPC_API_AVAILABLE:
            logger.error("june-grpc-api package not available. Cannot run gRPC load tests.")
            return
        
        logger.info(f"Starting gRPC load test with {self.num_users} users for {self.duration}s")
        ...
        for service in ["inference", "stt", "tts"]:
            ...
            if successful:
                ...
                if len(response_times) > 1:
                    print(f"  P95 response time: {self.percentile(response_times, 95):.2f}ms")
                    print(f"  P99 response time: {self.percentile(response_times, 99):.2f}ms")
...
async def main():
    ...
    parser.add_argument("--inference-host", default="localhost:50051", help="Inference API host")
    parser.add_argument("--stt-host", default="localhost:50052", help="STT service host")
    parser.add_argument("--tts-host", default="localhost:50053", help="TTS service host")
    parser.add_argument("--users", type=int, default=10, help="Number of concurrent users")
    parser.add_argument("--duration", type=int, default=60, help="Test duration in seconds")
    parser.add_argument("--spawn-rate", type=float, default=1.0, help="User spawn rate per second")
