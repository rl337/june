...
def generate_summary_report(metrics: dict, df: pd.DataFrame, output_path: Path):
    ...
    with open(report_path, "w") as f:
        ...
        if not df.empty:
            ...
            f.write("-" * 80 + "\n")
            f.write(
                f"{'Endpoint':<40} {'Avg RT (ms)':<15} {'P95 (ms)':<15} {'Errors':<10}\n"
            )
            f.write("-" * 80 + "\n")
            ...
            error_rate_violations = df[
                (df["num_failures"] / df["num_requests"] * 100) > 1
            ]
...
def compare_reports(baseline_path: Path, current_path: Path, output_path: Path):
    ...
    baseline_metrics, baseline_df = generate_metrics_report(
        baseline_data, output_path / "baseline"
    )
    current_metrics, current_df = generate_metrics_report(
        current_data, output_path / "current"
    )
...
def main():
    ...
    parser.add_argument("json_path", type=Path, help="Path to Locust JSON report")
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        "--compare", type=Path, help="Path to baseline JSON report for comparison"
    )
