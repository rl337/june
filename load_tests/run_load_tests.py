...
def run_locust_test(
    ...
    cmd = [
        ...
        "--run-time", duration,
        "--headless",
        "--html", str(output_dir / f"{report_name}.html"),
        "--csv", str(output_dir / report_name),
        "--json", str(output_dir / f"{report_name}.json"),
    ]
...
def run_grpc_test(
    ...
    cmd = [
        ...
        "--inference-host", grpc_config.get("inference_api", {}).get("host", "localhost:50051"),
        "--stt-host", grpc_config.get("stt", {}).get("host", "localhost:50052"),
        "--tts-host", grpc_config.get("tts", {}).get("host", "localhost:50053"),
    ]
    
    # Get scenario config (use first available or default)
    scenario_name = list(config.get("scenarios", {}).keys())[0] if config.get("scenarios") else "baseline"
    scenario = config.get("scenarios", {}).get(scenario_name, {})
    
    cmd.extend([
        "--users", str(scenario.get("users", 10)),
        "--duration", str(int(scenario.get("duration", "5m").rstrip("m")) * 60),
        "--spawn-rate", str(scenario.get("spawn_rate", 2)),
    ])
...
def main():
    ...
    parser.add_argument(
        "--host",
        type=str,
        help="Override Gateway host (default from config)"
    )
    ...
    # Get host
    host = args.host or config.get("services", {}).get("gateway", {}).get("host", "http://localhost:8000")
    
    # Create output directory
    output_dir = args.output_dir / scenario_name / datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir.mkdir(parents=True, exist_ok=True)
