...
1. **Deploy NIMs for inference:** ⏳ IN PROGRESS (HIGH PRIORITY - Agent should work on this)
   ...
   - **Steps:**
     ...
     - ⏳ Verify LLM NIM connectivity: `cd /home/rlee/dev/june && poetry run python -m essence verify-nim --nim-host nim-qwen3 --http-port 8003 --grpc-port 8001` → **PENDING** (waiting for model initialization to complete - **CRITICAL ISSUE (2025-11-21 00:30):** Service failed initialization with RuntimeError: "Engine core initialization failed" due to GPU memory error. Root cause: vLLM inside NIM container is hardcoded to use 0.9 (90%) GPU memory utilization, ignoring GPU_MEMORY_UTILIZATION environment variable. Error: "Free memory on device (102.36/119.7 GiB) on startup is less than desired GPU memory utilization (0.9, 107.73 GiB)". **FIX ATTEMPT (2025-11-21 00:30):** Reduced GPU_MEMORY_UTILIZATION from 0.80 to 0.70 and recreated service. **VERIFIED:** Environment variable is set correctly inside container (GPU_MEMORY_UTILIZATION=0.70), but vLLM is still using 0.9 internally. **ISSUE:** vLLM is not reading GPU_MEMORY_UTILIZATION env var - may need to find alternative configuration method (e.g., VLLM_GPU_MEMORY_UTILIZATION, or NIM-specific config) or check NIM/vLLM documentation for correct way to configure GPU memory usage. **STATUS:** Service recreated with 0.70, currently downloading model files (hasn't reached initialization phase yet). **NEXT:** Monitor for same error when initialization starts, or find correct vLLM/NIM configuration method.)
