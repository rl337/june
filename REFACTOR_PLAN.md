...
1. **Deploy NIMs for inference:** ⏳ IN PROGRESS (HIGH PRIORITY - Agent should work on this)
   ...
   - **Steps:**
     ...
     - ⏳ Verify LLM NIM connectivity: `cd /home/rlee/dev/june && poetry run python -m essence verify-nim --nim-host nim-qwen3 --http-port 8003 --grpc-port 8001` → **PENDING** (waiting for model initialization to complete - **CRITICAL:** All 5 safetensors downloaded (model-00004 completed at 23:55:49), but service failed initialization with GPU memory error. **FIX ATTEMPT (2025-11-20 15:59):** Reduced GPU_MEMORY_UTILIZATION from 0.85 to 0.80 and recreated service. Error: "Free memory on device (102.38/119.7 GiB) less than desired GPU memory utilization (0.9, 107.73 GiB)" - vLLM still using 0.9 despite environment variable. **ISSUE:** vLLM may not be reading GPU_MEMORY_UTILIZATION environment variable correctly - may need to check NIM/vLLM documentation for correct configuration method or reduce further.)
