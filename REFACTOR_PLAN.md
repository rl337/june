...
1. **Deploy NIMs for inference:** ⏳ IN PROGRESS (HIGH PRIORITY - Agent should work on this)
   ...
   - **Steps:**
     ...
     - ⏳ Verify LLM NIM connectivity: `cd /home/rlee/dev/june && poetry run python -m essence verify-nim --nim-host nim-qwen3 --http-port 8003 --grpc-port 8001` → **PENDING** (waiting for model initialization to complete - **STATUS (2025-11-20 17:34):** Service still initializing - CONTINUED PROGRESS! model-00001-of-00005.safetensors downloaded at 01:34:24. Currently confirmed downloaded: model-00001, model-00004, and model-00005 (3 of 5 files). Service is actively downloading model files - appears to be downloading in non-sequential order or re-downloading files that weren't properly cached. Python process active (13% CPU, 1.2% memory usage). Service in "unhealthy" state (expected during initialization). No engine initialization messages yet - service may be verifying cached files, loading model weights into GPU memory, or performing other initialization steps. Health check endpoint not responding yet. Service has been initializing for ~27 minutes. Remaining files to download: model-00002 and model-00003. **CRITICAL ISSUE (2025-11-21 00:30):** Service previously failed initialization with RuntimeError: "Engine core initialization failed" due to GPU memory error. Root cause: vLLM inside NIM container is hardcoded to use 0.9 (90%) GPU memory utilization, ignoring GPU_MEMORY_UTILIZATION environment variable. Error: "Free memory on device (102.3/119.7 GiB) on startup is less than desired GPU memory utilization (0.9, 107.73 GiB)". **FIX ATTEMPTS:** (1) Reduced GPU_MEMORY_UTILIZATION from 0.80 to 0.70 (2025-11-21 00:30). (2) Added VLLM_GPU_MEMORY_UTILIZATION environment variable (2025-11-21 00:36) - vLLM may use this instead. (3) Reduced GPU_MEMORY_UTILIZATION from 0.70 to 0.60 (2025-11-20 17:07) - attempting to work around vLLM ignoring env vars by setting lower value. **VERIFIED:** Environment variables set correctly inside container (GPU_MEMORY_UTILIZATION=0.60, VLLM_GPU_MEMORY_UTILIZATION=0.60), but vLLM still using 0.9 internally in previous attempts. **ISSUE:** vLLM is not reading GPU_MEMORY_UTILIZATION env var - may need to find alternative configuration method or check NIM/vLLM documentation for correct way to configure GPU memory usage. **NEXT:** Monitor remaining model file downloads (model-00002, model-00003) and engine initialization to see if 0.60 value works, or if vLLM still ignores env vars and uses 0.9. If same GPU memory error occurs, investigate vLLM configuration options or NIM-specific GPU memory settings. May need to check NIM documentation for correct way to configure GPU memory, or consider using a different NIM image version that respects environment variables.)
