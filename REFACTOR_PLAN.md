...
1. **Deploy NIMs for inference:** ⏳ IN PROGRESS (HIGH PRIORITY - Agent should work on this)
   ...
   - **Steps:**
     ...
     - ⏳ Verify LLM NIM connectivity: `cd /home/rlee/dev/june && poetry run python -m essence verify-nim --nim-host nim-qwen3 --http-port 8003 --grpc-port 8001` → **FAILED** (Service failed engine initialization - **CRITICAL FAILURE (2025-11-20 17:39):** All 5 model safetensors files downloaded successfully (model-00001, model-00002, model-00003, model-00004, model-00005). Service failed at 01:39:54 with RuntimeError: "Engine core initialization failed" due to GPU memory error. Root cause: vLLM inside NIM container is hardcoded to use 0.9 (90%) GPU memory utilization, ignoring GPU_MEMORY_UTILIZATION=0.60 and VLLM_GPU_MEMORY_UTILIZATION=0.60 environment variables. Error: "Free memory on device (102.85/119.7 GiB) on startup is less than desired GPU memory utilization (0.9, 107.73 GiB)". **FIX ATTEMPTS:** (1) Reduced GPU_MEMORY_UTILIZATION from 0.80 to 0.70 (2025-11-21 00:30). (2) Added VLLM_GPU_MEMORY_UTILIZATION environment variable (2025-11-21 00:36) - vLLM may use this instead. (3) Reduced GPU_MEMORY_UTILIZATION from 0.70 to 0.60 (2025-11-20 17:07) - attempting to work around vLLM ignoring env vars by setting lower value. (4) **ATTEMPTED (2025-11-20 17:39):** Stopped TensorRT-LLM service to free GPU memory - did not help. Free memory increased from 101.78 GiB to 102.85 GiB, but vLLM still wants 107.73 GiB (0.9 of 119.7 GiB). **VERIFIED:** Environment variables set correctly inside container (GPU_MEMORY_UTILIZATION=0.60, VLLM_GPU_MEMORY_UTILIZATION=0.60), but vLLM still using 0.9 internally. **ISSUE:** vLLM is not reading GPU_MEMORY_UTILIZATION env var - may need to find alternative configuration method or check NIM/vLLM documentation for correct way to configure GPU memory usage. **NEXT STEPS:** (1) Try reducing GPU_MEMORY_UTILIZATION further (0.50 or 0.55) - unlikely to help if vLLM is hardcoded to 0.9, (2) Investigate NIM/vLLM configuration options for GPU memory (check NIM documentation, vLLM command-line arguments, or config files), (3) Check if NIM passes environment variables to vLLM correctly - may need to configure vLLM directly, (4) Consider using different NIM image version or approach if vLLM configuration cannot be changed, (5) Consider using TensorRT-LLM instead of NIM if NIM cannot be configured properly.)
