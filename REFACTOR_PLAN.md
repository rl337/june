...
1. **Deploy NIMs for inference:** ⏳ IN PROGRESS (HIGH PRIORITY - Agent should work on this)
   ...
   - **Steps:**
     ...
     - ⏳ Verify LLM NIM connectivity: `cd /home/rlee/dev/june && poetry run python -m essence verify-nim --nim-host nim-qwen3 --http-port 8003 --grpc-port 8001` → **PENDING** (waiting for model initialization to complete - **CRITICAL:** All 5 safetensors downloaded (model-00004 completed at 23:55:49), but service failed initialization with GPU memory error. Service recreated 2025-11-20 15:57:38 to pick up GPU_MEMORY_UTILIZATION=0.85. Error: "Free memory on device (102.38/119.7 GiB) less than desired GPU memory utilization (0.9, 107.73 GiB)" - vLLM still using 0.9 despite environment variable set to 0.85. Service currently restarting/initializing. **ISSUE:** vLLM may not be reading GPU_MEMORY_UTILIZATION environment variable correctly - may need to reduce further or check vLLM config.)
