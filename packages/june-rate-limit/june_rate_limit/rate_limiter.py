...
class RateLimiter:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Connected to Redis for rate limiting at {self.config.redis_url}")
            return True
    ...
    async def check_rate_limit(
        ...
        if endpoint and endpoint in self.config.endpoint_limits:
            ...
            per_minute = per_minute or endpoint_config.get('per_minute', self.config.default_per_minute)
            per_hour = per_hour or endpoint_config.get('per_hour', self.config.default_per_hour)
            per_day = per_day or endpoint_config.get('per_day', self.config.default_per_day)
        else:
    ...
    async def _check_limit_redis(
        ...
        try:
            ...
            return RateLimitResult(
                ...
                error_message=None if allowed else f"Rate limit exceeded: {limit} requests per {window_seconds}s"
            )
    ...
    async def get_stats(self, identifier: str, identifier_type: str = "user") -> Dict[str, int]:
            ...
            if self._use_redis and self._is_connected:
                ...
                async with self._memory_lock:
                    ...
                    if memory_key in self.config._memory_storage:
                        ...
                        stats[limit_type] = len([ts for ts in requests if ts > window_start])
                    else:
                        stats[limit_type] = 0
