...
def whisper_strategy(mock_whisper_adapter):
    ...
    strategy = WhisperSttStrategy(
        model_name="tiny.en", device="cpu", whisper_adapter=mock_whisper_adapter
    )
    strategy.warmup()
    return strategy
...
def test_whisper_strategy_warmup_fails_on_import_error():
    ...
    with patch(
        "inference_core.stt.whisper_adapter.WhisperModelImpl",
...
def test_whisper_strategy_infer_handles_stereo_audio(
    whisper_strategy, mock_whisper_adapter
):
