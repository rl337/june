...
def test_passthrough_llm_strategy_infer_with_request():
    ...
    request = InferenceRequest(
        payload={"prompt": "hello", "params": {"max_tokens": 100}}, metadata={}
    )
    result = strategy.infer(request)
...
def test_qwen3_strategy_warmup_success(
    mock_torch, mock_tokenizer_class, mock_model_class
):
...
@patch("inference_core.llm.qwen3_strategy.AutoTokenizer")
@patch("inference_core.llm.qwen3_strategy.torch")
def test_qwen3_strategy_local_files_only(
    mock_torch, mock_tokenizer_class, mock_model_class
):
    ...
    strategy = Qwen3LlmStrategy(
        model_name="Qwen/Qwen3-30B-A3B", device="cpu", local_files_only=True
    )
    strategy.warmup()
...
def test_qwen3_strategy_warmup_missing_dependencies():
    ...
    with patch(
        "builtins.__import__", side_effect=ImportError("No module named 'transformers'")
    ):
...
def test_qwen3_strategy_infer_without_warmup():
    ...
    with pytest.raises(RuntimeError, match="Model not loaded"):
        strategy.infer(
            InferenceRequest(payload={"prompt": "test", "params": {}}, metadata={})
        )
...
@pytest.mark.skipif(
    not os.path.exists("/home/rlee/models")
    or not os.path.exists("/home/rlee/models/huggingface"),
...
def test_qwen3_strategy_model_parameters_configuration():
    ...
    try:
        ...
        if hasattr(model_config, "max_position_embeddings"):
            ...
            assert (
                model_config.max_position_embeddings >= strategy.max_context_length
                or (
                    hasattr(model_config, "rope_scaling")
                    and model_config.rope_scaling is not None
                )
            )
        ...
        if torch is not None:
            if strategy.device.startswith("cpu"):
                ...
                assert strategy._model.dtype in [
                    torch.float16,
                    torch.bfloat16,
                    torch.float32,
                ]
...
def test_qwen3_strategy_local_files_only_flag():
    ...
    with patch(
        "inference_core.llm.qwen3_strategy.AutoModelForCausalLM"
    ) as mock_model_class, patch(
        "inference_core.llm.qwen3_strategy.AutoTokenizer"
    ) as mock_tokenizer_class, patch(
        "inference_core.llm.qwen3_strategy.torch"
    ) as mock_torch:
        # Setup mocks
        mock_tokenizer = Mock()
        mock_tokenizer_class.from_pretrained.return_value = mock_tokenizer
