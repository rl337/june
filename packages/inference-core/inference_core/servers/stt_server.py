...
class _SttServicer(asr_pb2_grpc.SpeechToTextServicer):
    ...
    def Recognize(
        self, request: asr_pb2.RecognitionRequest, context
    ) -> asr_pb2.RecognitionResponse:
        ...
        r = resp.results.add()
        r.transcript = (
            result.payload if isinstance(result.payload, str) else str(result.payload)
        )
        r.confidence = (
            float(result.metadata.get("confidence", 0.0)) if result.metadata else 0.0
        )
        return resp
...
class SttGrpcApp:
    ...
    def run(self) -> None:
        ...
        asr_pb2_grpc.add_SpeechToTextServicer_to_server(
            _SttServicer(self.strategy), server
        )
