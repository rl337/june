...
class ModelProbe(Command):
    ...
    def add_args(self, subparser):
        subparser.add_argument(
            "--model", required=True, help="Model identifier to load"
        )
        subparser.add_argument(
            "--device", default="cpu", help="Device to target (cpu/cuda:0)"
        )
    ...
    def run(self, args) -> int:
        ...
        logger.info(
            "Model probe requested: name=%s device=%s", self.model_name, self.device
        )
        ...
        try:
            ...
            if "qwen" in self.model_name.lower():
                ...
                try:
                    ...
                    logger.info(
                        "? Inference successful: %s",
                        result.payload.get("text", "")[:50],
                    )
