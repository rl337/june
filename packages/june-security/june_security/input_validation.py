...
class InputValidator:
    ...
    ALLOWED_AUDIO_EXTENSIONS = {'.wav', '.mp3', '.ogg', '.flac', '.m4a', '.webm', '.opus'}
    ...
    def validate_file_upload(
        ...
        if '..' in validated_filename or '/' in validated_filename or '\\' in validated_filename:
            raise InputValidationError("Filename contains invalid characters (directory traversal attempt)")
        ...
        validated_mime_type = content_type or detected_mime_type or "application/octet-stream"
...
def get_input_validator(security_validator: Optional[SecurityValidator] = None) -> InputValidator:
    """
    ...
    elif security_validator is not None and _global_validator.security_validator is None:
