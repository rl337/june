...
class SecurityMonitor:
    ...
    def __init__(
        ...
        self.blocked_operations: Dict[str, deque] = defaultdict(lambda: deque(maxlen=100))
    ...
    def analyze_operation(
        ...
        # Check for threats
        threat = self._detect_threats(agent_id, operation, allowed, operation_type, timestamp)
    ...
    def _detect_threats(
        ...
        for pattern_checker in self.suspicious_patterns:
            threat = pattern_checker(agent_id, operation, allowed, operation_type, timestamp)
            if threat:
                return threat
    ...
    def _detect_rapid_failed_operations(
        ...
        recent_ops = [
            op for op in self.agent_operations[agent_id]
            if (timestamp - op['timestamp']).total_seconds() < self.time_window_minutes * 60
            and not op['allowed']
        ]
        ...
        if len(recent_ops) >= self.suspicious_pattern_threshold:
            return SecurityThreat(
                ...
                operation=operation,
                timestamp=timestamp,
                details={"failed_count": len(recent_ops), "time_window": self.time_window_minutes}
            )
    ...
    def _detect_repeated_blocked_operations(
        ...
        if not allowed:
            ...
            recent_blocked = [
                op for op in self.blocked_operations[agent_id]
                if (timestamp - op['timestamp']).total_seconds() < self.time_window_minutes * 60
            ]
    ...
    def _detect_mass_file_deletions(
        ...
        if 'rm' in operation.lower() or 'delete' in operation.lower():
            ...
            recent_ops = [
                op for op in self.agent_operations[agent_id]
                if (timestamp - op['timestamp']).total_seconds() < self.time_window_minutes * 60
                and ('rm' in op['operation'].lower() or 'delete' in op['operation'].lower())
            ]
