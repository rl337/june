...
class ValidationResult:
    ...
    def __init__(
        self, allowed: bool, reason: Optional[str] = None, severity: str = "error"
    ):
        self.allowed = allowed
        self.reason = reason or (
            "Operation allowed" if allowed else "Operation blocked"
        )
...
class SecurityValidator:
    ...
    def __init__(
        ...
        logger.info(
            f"SecurityValidator initialized with {len(self.allowed_project_paths)} allowed project paths"
        )

    def validate_command(
        self, command: str, operation_type: OperationType = OperationType.COMMAND
    ) -> ValidationResult:
        """
        ...
        if operation_type == OperationType.FILE_DELETE:
            ...
            if "rm" in command.lower() and not self._is_safe_delete_command(command):
                return ValidationResult(False, "Unsafe file deletion command", "error")

        return ValidationResult(True, "Command validation passed")

    def validate_file_path(
        self, file_path: str, operation_type: OperationType = OperationType.FILE_READ
    ) -> ValidationResult:
        """
        ...
        if "commit" in git_command_lower:
            ...
            if commit_msg_match:
                ...
                if commit_msg.lower().strip() in suspicious_patterns:
                    return ValidationResult(
                        False, f"Commit message too generic: '{commit_msg}'", "warning"
                    )
    ...
    def validate_operation(
        self, operation: Dict[str, Any], operation_type: Optional[OperationType] = None
    ) -> ValidationResult:
        """
        ...
        if operation_type is None:
            if "git_command" in operation or (
                "command" in operation and "git" in operation.get("command", "").lower()
            ):
                operation_type = OperationType.GIT_OPERATION
            elif "file_path" in operation:
                if (
                    "write" in operation.get("action", "").lower()
                    or "write" in operation.get("operation", "").lower()
                ):
                    operation_type = OperationType.FILE_WRITE
                elif (
                    "delete" in operation.get("action", "").lower()
                    or "delete" in operation.get("operation", "").lower()
                ):
        ...
        if "git_command" in operation:
            result = self.validate_git_operation(
                operation["git_command"], operation_type
            )
            results.append(result)
            if not result:
                return result
