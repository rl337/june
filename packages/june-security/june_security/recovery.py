...
class RecoveryManager:
    ...
    def rollback_git_state(
        self, project_path: str, commit_hash: Optional[str] = None
    ) -> bool:
        """
    ...
    def cleanup_old_backups(self, keep_count: int = 10) -> int:
        ...
        sorted_backups = sorted(
            self.backups.items(), key=lambda x: x[1]["created_at"], reverse=True
        )
    ...
    def auto_backup_before_destructive_operation(
        self, project_path: str, operation_description: str
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            logger.info(
                f"Auto-backup created: {backup_id} for operation: {operation_description}"
            )
            return backup_id
        except Exception as e:
    ...
    def auto_rollback_on_failure(
        self, project_path: str, backup_id: Optional[str] = None
    ) -> bool:
        """
        ...
        if backup_id:
            ...
            try:
                return self.restore_backup(
                    backup_id, restore_path=project_path, overwrite=True
                )
            except Exception as e:
                logger.error(
                    f"Failed to restore from backup {backup_id}: {e}", exc_info=True
                )
                return False
        else:
            # Try git rollback
