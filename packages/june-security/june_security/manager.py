...
class SecurityManager:
    ...
    def validate_operation(
        ...
        operation_str = (
            operation.get('command') or
            operation.get('git_command') or
            operation.get('file_path') or
            str(operation)
        )
        ...
        if not validation_result and self.audit_logger:
            ...
            self.audit_logger.log_security_violation(
                ...
                operation=operation_str,
                details={"reason": validation_result.reason, "threat": str(threat) if threat else None}
            )
