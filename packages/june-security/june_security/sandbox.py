...
class SandboxManager:
        ...
        try:
            ...
            if self.enable_resource_limits:
                try:
                    ...
                    resource.setrlimit(
                        resource.RLIMIT_CPU, (cpu_time_seconds, cpu_time_seconds)
                    )
            ...
            if project_path and Path(project_path).exists():
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to copy project to sandbox: {e}", exc_info=True
                    )
            ...
            try:
                ...
                logger.error(
                    f"Failed to cleanup sandbox {sandbox_id}: {e}", exc_info=True
                )
    ...
    def _copy_project_to_sandbox(self, source: Path, destination: Path):
        ...
        for item in source.iterdir():
            ...
            if item.is_dir():
                shutil.copytree(
                    item, dest_item, ignore=shutil.ignore_patterns(*exclude_patterns)
                )
            else:
