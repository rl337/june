...
class CodingAgent:
    ...
    def send_coding_task(
        ...
        with tracer.start_as_current_span("coding_agent.send_task") as span:
            try:
                ...
                span.set_attribute("conversation_length", len(self._conversation_history))
                ...
                with tracer.start_as_current_span("coding_agent.stream_response") as stream_span:
                    for chunk in self._stub.ChatStream(request):
                        if chunk.chunk.role == "assistant":
                            ...
                            if chunk.chunk.tool_calls:
                                ...
                                stream_span.set_attribute("tool_calls_count", len(tool_calls))
                ...
                if full_response:
                    assistant_message = self._create_chat_message("assistant", full_response)
                    if tool_calls:
                ...
                raise RuntimeError(f"Failed to communicate with inference API: {e}") from e
    ...
    def _build_system_message(self, context: Optional[Dict[str, Any]]) -> Optional[str]:
        ...
        """
        if not context:
            return "You are a helpful coding assistant. Write clean, well-documented code."
        
        parts = ["You are a helpful coding assistant. Write clean, well-documented code."]
    ...
    def _execute_tool_calls(self, tool_calls: List[ToolCall], parent_span: trace.Span) -> Dict[str, Any]:
        """
    ...
    def _tool_write_file(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            with open(full_path, "w", encoding="utf-8") as f:
                f.write(content)
            return {"success": True, "file_path": file_path, "bytes_written": len(content.encode("utf-8"))}
        except Exception as e:
    ...
    def _tool_list_files(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        target_dir = self._workspace_dir / directory if directory else self._workspace_dir
    ...
    def _tool_execute_command(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if working_dir:
            ...
            try:
                ...
                return {"error": f"Working directory {working_dir} is outside workspace"}
        else:
