...
def should_use_agentic_flow(
    ...
    if len(user_message) > complexity_threshold:
        logger.debug(f"Agentic flow: message length ({len(user_message)}) exceeds threshold ({complexity_threshold})")
        return True
    
    # Check if tools are likely needed
    tool_keywords = ["file", "code", "write", "create", "modify", "execute", "run", "implement"]
    ...
    if message_history and len(message_history) > 3:
        logger.debug(f"Agentic flow: multi-turn conversation ({len(message_history)} messages)")
        return True
...
def estimate_request_complexity(
    ...
    complex_keywords = ["multiple", "several", "complex", "advanced", "sophisticated", "comprehensive"]
    moderate_keywords = ["create", "modify", "update", "analyze", "process", "implement"]
    
    complexity_score += sum(2 for keyword in complex_keywords if keyword in user_message.lower())
    complexity_score += sum(1 for keyword in moderate_keywords if keyword in user_message.lower())
