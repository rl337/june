...
def analyze_rendering_issues(
    ...
    """
    messages = get_recent_messages(user_id=user_id, chat_id=chat_id, platform=platform, hours=hours)
    ...
    for msg in messages:
        ...
        if metadata.get("was_truncated"):
            ...
            analysis["issues"].append({
                ...
                "max_length": metadata.get("telegram_max_length") or metadata.get("discord_max_length")
            })
...
def compare_expected_vs_actual(
    ...
    comparison = {
        ...
        "formatted_length": len(best_match.formatted_text) if best_match.formatted_text else 0,
...
def validate_message_for_platform(
    ...
    if platform == "telegram":
        ...
        if "&" in text and "&amp;" not in text and "&lt;" not in text and "&gt;" not in text:
            result["warnings"].append("Unescaped '&' character may cause issues in HTML mode")
...
def get_message_statistics(
    ...
    # Count by platform
    for msg in messages:
        stats["by_platform"][msg.platform] = stats["by_platform"].get(msg.platform, 0) + 1
