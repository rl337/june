...
def setup_tracing(
    service_name: str = "june-telegram", jaeger_endpoint: Optional[str] = None
):
    """
    ...
    try:
        ...
        collector_endpoint = os.getenv(
            "JAEGER_COLLECTOR_ENDPOINT", f"http://{jaeger_agent_host}:14268/api/traces"
        )
        ...
        if GRPC_INSTRUMENTATION_AVAILABLE:
            ...
            try:
                ...
                logger.info(
                    "gRPC instrumentation enabled for automatic trace context propagation"
                )
            except Exception as e:
                logger.warning(
                    f"Failed to enable gRPC instrumentation: {e}. Trace context may not propagate across gRPC calls."
                )
        else:
            logger.warning(
                "opentelemetry-instrumentation-grpc not available. Trace context may not propagate across gRPC calls."
            )

        logger.info(
            f"Tracing initialized for service '{service_name}' with Jaeger endpoint: {jaeger_endpoint}"
        )
    except Exception as e:
        logger.warning(
            f"Failed to initialize tracing: {e}. Continuing without tracing."
        )
