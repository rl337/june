...
def _get_agentic_reasoner() -> Optional[AgenticReasoner]:
    ...
    if _agentic_reasoner is None:
        try:
            ...
            executor = Executor(
                available_tools={}
            )  # No tools for chat handler (coding agent has tools)
            ...
            logger.warning(
                f"Failed to initialize agentic reasoner: {e}. Will use direct flow only."
            )
            return None
    ...
    if result.success:
        return {
            ...
            "raw_response": {
                ...
                "reflection": {
                    "goal_achieved": result.reflection.goal_achieved
                    if result.reflection
                    else None,
                    "confidence": result.reflection.confidence
                    if result.reflection
                    else None,
                    "issues": result.reflection.issues_found
                    if result.reflection
                    else [],
                }
                if result.reflection
...
def find_agenticness_directory(
    script_name: str = "telegram_response_agent.sh",
) -> Optional[str]:
    """
    ...
    with tracer.start_as_current_span("agent.process_message") as span:
        try:
            ...
            context = _build_conversation_context(
                user_id, chat_id, platform, user_message
            )
            ...
            if reasoner:
                try:
                    ...
                    logger.warning(
                        f"Error in decision logic: {e}. Falling back to direct flow."
                    )
                    use_agentic_flow = False
            ...
            if use_agentic_flow and reasoner:
                try:
                    ...
                    logger.info(
                        f"Using agentic reasoning flow for user {user_id}, chat {chat_id}"
                    )
                    ...
                    formatted_message = response_data.get(
                    ...
                    logger.error(
                        f"Error in agentic reasoning flow: {e}. Falling back to direct flow.",
                        exc_info=True,
                    )
            ...
            logger.debug(
                f"Using direct response flow for user {user_id}, chat {chat_id}"
            )
            ...
            if agenticness_dir is None:
                ...
                span.set_status(
                    trace.Status(
                        trace.StatusCode.ERROR, "Agent system not properly configured"
                    )
                )
                return {
                    "success": False,
