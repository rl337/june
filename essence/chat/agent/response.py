...
def call_chat_response_agent(
    ...
    if user_id is not None and chat_id is not None:
        ...
        logger.warning(
            "user_id and chat_id not provided - using simple script without session support"
        )
        agent_script = os.path.join(
            agenticness_dir, "scripts", agent_script_simple_name
        )
    ...
    try:
        ...
        env["TODO_SERVICE_URL"] = env.get(
            "TODO_SERVICE_URL", "http://localhost:8000/mcp/todo-mcp-service"
        )
        ...
        agenticness_state_dir = os.getenv(
            "AGENTICNESS_STATE_DIR", "/home/rlee/june_data/agenticness-state"
        )
        ...
        if user_id is not None and chat_id is not None:
            ...
            logger.info(
                f"Using session-aware agent for user {user_id}, chat {chat_id}, mode: {agent_mode}"
            )
        ...
        logger.info(
            f"Calling {platform} response agent (mode: {agent_mode}) with message: {cleaned_message[:50]}..."
        )
        import pty
        import select
        ...
        if pid == 0:
            ...
            try:
                ...
                while time.time() - start_time < timeout:
                    ...
                    try:
                        ...
                        if wait_result[0] == pid:
                            ...
                            logger.debug(
                                "Child process finished, reading remaining output"
                            )
                            ...
                            while remaining_attempts > 0:
                                ...
                                try:
                                    ...
                                    if data:
                                        output_chunks.append(data)
                                        logger.debug(
                                            f"Read {len(data)} remaining bytes"
                                        )
                                    remaining_attempts -= 1
                ...
                logger.debug(
                    f"Agent completed with return code {returncode}, output length: {len(stdout)}"
                )
        ...
        if result.returncode != 0:
            ...
            logger.error(
                f"Agent stdout: {result.stdout[:1000] if result.stdout else '(empty)'}"
            )
        ...
        try:
                    ...
                    if obj_type == "tool_call" and subtype == "completed":
                        ...
                        if "editToolCall" in tool_call:
                            ...
                            if "success" in edit_result:
                                file_content = edit_result["success"].get(
                                    "afterFullFileContent", ""
                                )
                                if (
                                    file_content
                                    and isinstance(file_content, str)
                                    and len(file_content.strip()) > 10
                                ):
                                    # Filter out descriptions
                                    if not file_content.strip().startswith(
                                        ("Writing", "Wrote", "Created", "Updated")
                                    ):
            ...
            if final_result_text:
                ...
                return {"message": final_result_text, "response": final_result_text}
            else:
            ...
            logger.error(
                f"Failed to parse agent stream-json output: {e}", exc_info=True
            )
...
def stream_chat_response_agent(
    ...
    if not os.access(agent_script, os.X_OK):
        logger.error(f"Agent script is not executable: {agent_script}")
        yield (
            f"⚠️ The {platform} response agent script is not executable.",
            True,
            None,
        )
        return
    ...
    if user_id is not None and chat_id is not None:
        ...
        env[f"{platform.upper()}_CHAT_ID"] = str(chat_id)
        logger.info(
            f"Streaming agent response for user {user_id}, chat {chat_id}, mode: {agent_mode}"
        )
    ...
    try:
        ...
        for line, is_final_line in streaming_popen_generator(
            command, env=env, chunk_size=1024, read_timeout=0.05
        ):
            ...
            if elapsed > max_total_time:
                logger.warning(
                    f"Agent stream exceeded max total time ({max_total_time}s)"
                )
                yield (
                    "⏱️ The request took too long to process. Please try a simpler request.",
                    True,
                    None,
                )
                break
            ...
            if not line or not line.startswith("{"):
                ...
                if incomplete_json_buffer:
                    ...
                    try:
                        ...
                        if span:
                            span.add_event(
                                "buffer_parse_success",
                                attributes={"total_length": len(line)},
                            )
                        ...
                        if is_final_line:
                            ...
                            try:
                                ...
                                if span:
                                    span.add_event(
                                        "incomplete_json_line_final",
                                        attributes={
                                            "line_preview": incomplete_json_buffer[
                                                :200
                                            ],
            ...
            if not line or not line.startswith("{"):
                ...
                try:
                    ...
                    logger.debug(
                        f"Successfully parsed JSON line #{json_line_count}: type={obj_type}"
                    )
                    if span:
                        span.add_event(
                            "json_parse_success",
                    ...
                    incomplete_json_buffer = line
                    logger.debug(
                        f"JSON parse failed (incomplete?), accumulating: {len(line)} chars, error: {str(e)[:50]}"
                    )
                    if span:
                        span.add_event(
                            "json_parse_failed_accumulating",
                            attributes={
                                "line_length": len(line),
                                "error": str(e)[:100],
                                "is_final": is_final_line,
                    ...
                    if is_final_line:
                        ...
                        try:
                            ...
                            if span:
                                span.add_event(
                                    "json_parse_success_final_retry",
                                    attributes={
                                        "line_number": json_line_count,
                            ...
                            logger.debug(
                                f"Skipping incomplete JSON on final line: {len(incomplete_json_buffer)} chars"
                            )
                            incomplete_json_buffer = ""
                            # Don't continue - let final line logic handle it
                        ...
                        if span:
                            span.add_event(
                                "continuing_accumulation_after_fail",
                                attributes={
                                    "buffer_length": len(incomplete_json_buffer)
                                },
                            )
            ...
            if json_obj is not None:
                ...
                obj_type = json_obj.get("type", "")
                is_result = (
                    obj_type == "result" and json_obj.get("subtype") == "success"
                )
                ...
                if span:
                    if is_result:
                        ...
                        span.add_event(
                            ...
                            attributes={
                                "message_preview": str(
                                    json_obj.get("message", {}).get("content", [])
                                )[:200]
                            },
                        )
                ...
                if message:
                    ...
                    if is_result:
                        ...
                        if (
                            old_accumulated
                            and old_accumulated != message
                            and not first_message_sent
                        ):
                            logger.info(
                                f"Yielding assistant message before result: {len(old_accumulated)} chars"
                            )
                            if span:
                                span.add_event(
                                    "yielding_assistant_before_result",
                                    attributes={
                        ...
                        if accumulated_message:
                            ...
                            if accumulated_message in message:
                                ...
                                accumulated_message
                            ) * 0.8 and message.startswith(
                                accumulated_message[:20]
                                if len(accumulated_message) >= 20
                                else accumulated_message
                            ):
                                ...
                                logger.info(
                                    f"Detected full accumulated message (long restart pattern): {len(accumulated_message)} -> {len(message)} chars"
                                )
                                accumulated_message = message
                                message_updated = True
                            else:
                                ...
                                message_starts_with_space = (
                                    message and len(message) > 0 and message[0] == " "
                                )
                                is_single_punctuation = len(
                                    message.strip()
                                ) == 1 and message.strip() in (
                                    "!",
                                    "?",
                                    ".",
                                    ",",
                                ...
                                is_likely_continuation = (
                                    message_starts_with_space
                                    or (  # Starts with space (common continuation pattern)
                                        len(message.strip()) < 10
                                        and accumulated_message
                                        and not accumulated_message.rstrip().endswith(
                                            (" ", ".", ",", ":", ";", "!", "?")
                                        )
                                    )
                                    or (  # Short chunk that continues naturally
                                        len(message.strip()) == 1
                                        and message.strip()
                                        in ("!", "?", ".", ",", ":", ";")
                                    )  # Single punctuation
                                ...
                                starts_with_capital = (
                                    message_stripped and message_stripped[0].isupper()
                                    if message_stripped
                                    else False
                                )
                                is_fragment = (
                                    len(message_stripped) < 15
                                    and not starts_with_capital  # Very short
                                    and not message_starts_with_space  # Doesn't start with capital (after stripping)
                                    and not is_single_punctuation  # Doesn't start with space (space-prefixed chunks are continuations)
                                    and not message_stripped.startswith(  # Not single punctuation (these are valid continuations)
                                        (
                                            "1.",
                                            "2.",
                                            "3.",
                                    ...
                                    and not accumulated_message.rstrip().endswith(  # Not a list/code start
                                        (" ", ".", ",", ":", ";", "!", "?")
                                    )
                                    and not is_likely_continuation  # Doesn't continue naturally  # Not a likely continuation
                                )

                                if is_fragment:
                                    logger.debug(
                                        f"Skipping fragment chunk: {message[:50]}... (doesn't fit naturally)"
                                    )
                                    message_updated = False
                                elif len(message) < 20 and accumulated_message.endswith(
                                    message[: min(10, len(message))]
                                ):
                                    # Very short chunk that matches the end - likely a duplicate, skip it
                                    logger.debug(
                                        f"Skipping likely duplicate chunk: {message[:50]}... (matches end of accumulated)"
                                    )
                                    message_updated = False
                                elif (
                                    accumulated_message[-20:].strip()
                                    and message[:20].strip()
                                    and accumulated_message[-20:].strip()
                                    == message[:20].strip()
                                ):
                                    # Chunk starts with text that's already at the end - duplicate/restart, skip it
                                    logger.debug(
                                        f"Skipping duplicate/restart chunk: starts with text already at end"
                                    )
                                    message_updated = False
                                else:
                                    ...
                                    if old_accumulated and old_accumulated not in [
                                        msg for msg, _ in previous_accumulated_messages
                                    ]:
                                        previous_accumulated_messages.append(
                                            (old_accumulated, current_time)
                                        )
                                        logger.debug(
                                            f"Added old accumulated to history before update: length={len(old_accumulated)}"
                                        )
                                    ...
                                    if (
                                        ...
                                        not in [
                                            msg
                                            for msg, _ in previous_accumulated_messages
                                        ]
                                    ):
                                        previous_accumulated_messages.append(
                                            (accumulated_message, current_time)
                                        )
                                        logger.debug(
                                            f"Added new accumulated to history after update: length={len(accumulated_message)}"
                                        )
                                    logger.info(
                            ...
                            is_fragment = (
                                len(message) < 50
                                and not message[  # Short chunks are often fragments
                                    0
                                ].isupper()
                                and not message.strip().startswith(  # Doesn't start with capital
                                    ("1.", "2.", "3.", "-", "*", "#", ">", "`")
                                )  # Not a list/code start
                            )

                            if is_fragment:
                                logger.debug(
                                    f"Skipping first chunk (likely fragment): {message[:50]}..."
                                )
                                message_updated = False
                            else:
                                accumulated_message = message
                                message_updated = True
                                logger.info(
                                    f"First assistant chunk: {len(accumulated_message)} chars"
                                )
                                # Add first message to history immediately
                                if accumulated_message and accumulated_message not in [
                                    msg for msg, _ in previous_accumulated_messages
                                ]:
                                    previous_accumulated_messages.append(
                                        (accumulated_message, current_time)
                                    ...
                                    logger.debug(
                                        f"Added first message to history: length={len(accumulated_message)}"
                                    )
                    ...
                    if message_updated:
                        ...
                        if is_result:
                            ...
                            if accumulated_message and accumulated_message not in [
                                ...
                                logger.debug(
                                    f"Added accumulated message to history: length={len(accumulated_message)}, history_size={len(previous_accumulated_messages)}"
                                )
                            ...
                            if (
                                ...
                                and elapsed >= max_wait_for_first_message
                            ):
                                first_message_sent = True
                                ...
                                for msg, msg_time in previous_accumulated_messages:
                                    logger.info(
                                        f"Yielding accumulated message from history: length={len(msg)}, type={message_type}"
                                    )
                                    if span:
                                        span.add_event(
                                            "yielding_accumulated_from_history",
                                            attributes={
                                                "length": len(msg),
                                                "type": message_type,
                                            },
                                ...
                                if (
                                    last_yielded_msg
                                    and accumulated_message != last_yielded_msg
                                ):
                                    logger.info(
                                        f"Yielding current message after history: length={len(accumulated_message)}, type={message_type}"
                                    )
                                    ...
                                    logger.info(
                                        f"Yielding first message after {elapsed:.2f}s: length={len(accumulated_message)}, type={message_type}"
                                    )
                                    if span:
                                        span.add_event(
                                            "yielding_first_message",
                                            attributes={
                                ...
                                logger.info(
                                    f"Yielding updated accumulated message: length={len(accumulated_message)}, type={message_type}"
                                )
                                if span:
                                    span.add_event(
                                        "yielding_updated_message",
                                        attributes={
                                            "length": len(accumulated_message),
                                            "type": message_type,
                                            "is_result": is_result,
            ...
            if pending_messages:
                ...
                if time_since_last_yield >= min_message_interval:
                    ...
                    first_message_sent = True
                    logger.info(
                        f"Yielding pending message after {time_since_last_yield:.2f}s: length={len(message)}"
                    )
                    yield (
                        message,
                        False,
                        "assistant",
                    )  # Pending messages are from assistant chunks
                    last_message_yield_time = current_time

            # If this is the final line from the generator, process remaining messages
            if is_final_line:
                logger.info(
                    f"Received final line from generator. pending_messages={len(pending_messages)}, seen_messages={len(seen_messages)}, history_size={len(previous_accumulated_messages)}"
                )
                if span:
                    span.add_event(
                        "final_line_received",
                ...
                if not first_message_sent and elapsed >= max_wait_for_first_message:
                    ...
                    if accumulated_message and accumulated_message not in [
                        ...
                        logger.info(
                            f"Added current accumulated message to history (final line): length={len(accumulated_message)}"
                        )

                    # Yield all previous accumulated messages in sequence
                    # Sort by timestamp to ensure correct order
                    sorted_history = sorted(
                        previous_accumulated_messages, key=lambda x: x[1]
                    )
                    last_yielded_msg = None
                    for msg, msg_time in sorted_history:
                        logger.info(
                            f"Yielding accumulated message from history (final line): length={len(msg)}, timestamp={msg_time}"
                        )
                        if span:
                            span.add_event(
                                "yielding_accumulated_from_history_final",
                                attributes={"length": len(msg), "timestamp": msg_time},
                            )
                ...
                for message, _ in pending_messages:
                    if (
                        message
                        and message.strip()
                        and (message not in seen_messages or not first_message_sent)
                    ):
                        logger.info(
                            f"Yielding remaining pending message: length={len(message)}"
                        )
                        if span:
                            span.add_event(
                                "yielding_pending_message",
                                attributes={"length": len(message)},
                            )
                        yield (
                            message,
                            False,
                ...
                if (
                    time_since_last_json > line_timeout
                    and last_json_line_time > start_time
                ):
                    logger.warning(
                        f"No JSON line received for {time_since_last_json}s before final line"
                    )
                    if span:
                        span.add_event(
                            "json_timeout_warning",
                            attributes={"time_since_last_json": time_since_last_json},
                        )
                ...
                if span:
                    ...
                    span.set_attribute(
                        "result_message_received", result_message_received
                    )
                    span.set_attribute(
                        "accumulated_message_length", len(accumulated_message)
                    )
                ...
                if result_message:
                    logger.info(
                        f"Yielding final authoritative result message: {len(result_message)} chars"
                    )
                    if span:
                        span.add_event(
                            "yielding_final_result",
                            attributes={"length": len(result_message)},
                        )
                    yield (result_message, True, "result")
                elif first_message_sent and (
                    accumulated_message or json_line_count > 0
                ):
                    # We've already sent messages, just mark as final
                    logger.info(
                        f"Yielding final signal. Total messages sent: {len(seen_messages)}"
                    )
                    if span:
                        span.add_event(
                            "yielding_final_signal",
                            attributes={"total_messages": len(seen_messages)},
                        )
                    yield (
                        "",
                        True,
                        None,
                    )  # Empty message with is_final=True to signal completion
                else:
                    # No messages were extracted, send error
                    logger.warning("No messages were extracted from stream")
                    if span:
                        span.add_event(
                            "no_messages_extracted",
                            attributes={
                                "json_lines_processed": json_line_count,
                                "accumulated_length": len(accumulated_message),
                    ...
                    yield (
                        "⚠️ I received your message but couldn't generate a response. Please try again.",
                        True,
                        None,
                    )
                break
    ...
    try:
                ...
                if isinstance(content, list):
                    for item in content:
                        if isinstance(item, dict) and item.get("type") == "text":
                            ...
                            is_single_punctuation = len(
                                text_stripped
                            ) == 1 and text_stripped in ("!", "?", ".", ",", ":", ";")
                            if text_stripped and (
                                len(text_stripped) >= 5 or is_single_punctuation
                            ):  # Allow 5+ character messages or single punctuation
        ...
        if obj_type == "result" and subtype == "success":
            ...
            if (
                result_text
                and isinstance(result_text, str)
                and len(result_text.strip()) >= 5
            ):  # Allow 5+ character messages
                # Filter out descriptions
                if not result_text.strip().startswith(
                    ("Writing", "Wrote", "Created", "Updated", "Response written")
                ):
        ...
        for field in ["message", "content", "text", "response"]:
            ...
            if (
                value and isinstance(value, str) and len(value.strip()) >= 5
            ):  # Allow 5+ character messages
