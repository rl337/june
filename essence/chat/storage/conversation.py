...
class ConversationStorage:
    ...
    def get_language_preference(user_id: str, chat_id: str) -> str:
        ...
        try:
            ...
            logger.error(
                f"Error getting language preference for {user_id}/{chat_id}: {e}",
                exc_info=True,
            )
    ...
    def set_language_preference(user_id: str, chat_id: str, language_code: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                language_code = (
                    language_code.lower() if language_code else DEFAULT_LANGUAGE
                )
                ...
                if result:
                    ...
                    conn.commit()
                    logger.info(
                        f"Updated language preference for {user_id}/{chat_id}: {language_code}"
                    )
                    return True
                else:
                    ...
                    cursor.execute(
                        ...
                        (
                            str(user_id),
                            str(chat_id),
                            json.dumps({"language_preference": language_code}),
                        ),
                    )
                    conn.commit()
                    logger.info(
                        f"Created conversation with language preference for {user_id}/{chat_id}: {language_code}"
                    )
                    return True
            ...
            logger.error(
                f"Error setting language preference for {user_id}/{chat_id}: {e}",
                exc_info=True,
            )
            return False
        ...
        try:
            ...
            try:
                ...
                if result and result["metadata"]:
                    ...
                    if preferences:
                        logger.debug(
                            f"User preferences for {user_id}/{chat_id}: {preferences}"
                        )
                    return preferences
            ...
            logger.error(
                f"Error getting user preferences for {user_id}/{chat_id}: {e}",
                exc_info=True,
            )
        ...
        try:
            ...
            try:
                ...
                if result:
                    ...
                    conn.commit()
                    logger.info(
                        f"Updated user preferences for {user_id}/{chat_id}: name={name}, favorite_color={favorite_color}"
                    )
                    return True
                else:
                    # Conversation doesn't exist - create it with metadata
                    ...
                    conn.commit()
                    logger.info(
                        f"Created conversation with user preferences for {user_id}/{chat_id}: name={name}, favorite_color={favorite_color}"
                    )
                    return True
            ...
            logger.error(
                f"Error setting user preferences for {user_id}/{chat_id}: {e}",
                exc_info=True,
            )
            return False
    ...
    def get_conversation_analytics(user_id: str, chat_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                for msg in messages:
                    ...
                    if msg["role"] == "user":
                        ...
                        response_time = (
                            msg_time - last_user_message_time
                        ).total_seconds()
                ...
                if isinstance(first_message_time, str):
                    first_message_time = datetime.fromisoformat(
                ...
                if conversation_duration > 0:
                    ...
                    response_ratio = (
                        assistant_message_count / user_message_count
            ...
            logger.error(
                f"Error getting conversation analytics for {user_id}/{chat_id}: {e}",
                exc_info=True,
            )
            return {
                "error": str(e),
    ...
    def get_prompt_template_for_conversation(
        ...
        try:
            ...
            logger.error(
                f"Error getting conversation prompt template: {e}", exc_info=True
            )
    ...
    def assign_ab_variant(
        ...
        try:
            ...
            try:
                ...
                if existing:
                    logger.debug(
                        f"Conversation {conversation_id} already assigned to variant {existing['variant_name']}"
                    )
                ...
                if force_variant:
                    ...
                    if force_variant not in variant_names:
                        logger.error(
                            f"Force variant {force_variant} not found in test variants"
                        )
                        return None
                ...
                if test.get("traffic_split", 1.0) < 1.0:
                    ...
                    if split_hash > test["traffic_split"]:
                        logger.debug(
                            f"Conversation {conversation_id} excluded from test due to traffic_split"
                        )
                        return None
                ...
                conn.commit()
                logger.info(
                    f"Assigned conversation {conversation_id} to variant {selected_variant} for test {test_id}"
                )
    ...
    def record_ab_metric(
        ...
        try:
            ...
            try:
                ...
                conn.commit()
                logger.debug(
                    f"Recorded metric {metric_type}={metric_value} for variant {variant_name} in test {test_id}"
                )
                return True
            except Exception as e:
                conn.rollback()
