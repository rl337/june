...
class MarkdownParser:
    ...
    def _parse_text_blocks(self, text: str) -> List[ContentWidget]:
        ...
        for para_text in paragraphs:
            ...
            has_formatting = (
                self.bold_pattern.search(para_text)
                or self.italic_pattern.search(para_text)
                or self.inline_code_pattern.search(para_text)
                or self.link_pattern.search(para_text)
            )
    ...
    def _parse_table_lines(self, lines: List[str]) -> Optional[TableWidget]:
        ...
        for line in lines:
            stripped = line.strip()
            if not stripped.startswith("|") or self.table_separator_pattern.match(
                stripped
            ):
                continue
    ...
    def _parse_list_items(
        self, items: List[tuple], ordered: bool
    ) -> Optional[ListWidget]:
