    ...
    def to_dict(self) -> Dict[str, Any]:
        def row_to_dict(row: TableRow) -> Dict[str, Any]:
            return {
                ...
                "cells": [{"text": cell.text, "align": cell.align} for cell in row.cells]
            }
        return {
            "type": self.content_type.value,
...
class ErrorMessage:
    ...
    @classmethod
    def from_exception(cls, exception: Exception, user_message: Optional[str] = None) -> 'ErrorMessage':
...
def _widget_from_dict(data: Dict[str, Any]) -> Optional[ContentWidget]:
    ...
    if widget_type == ContentType.ESCAPED_TEXT.value:
        ...
        def dict_to_row(row_data: Dict[str, Any]) -> TableRow:
            return TableRow(
                cells=[TableCell(text=cell["text"], align=cell.get("align")) 
