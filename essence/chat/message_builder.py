...
class MessageBuilder:
    ...
    def build_turn(
        self, user_request: str, llm_response: str, metadata: Optional[dict] = None
    ) -> Turn:
        """
    ...
    def split_message_if_needed(
        self, max_length: int = 4096, message: Optional[Message] = None
    ) -> List[str]:
        """
        ...
        if len(rendered) <= 2 * max_length:
            ...
            return [rendered[:split_point].rstrip(), rendered[split_point:].lstrip()]
        ...
        if len(second_message) > max_length:
            second_message = (
                second_message[: max_length - truncation_len] + truncation_indicator
            )
        return [first_message, second_message]

    def _split_at_widget_boundaries(
        self, message: Message, max_length: int
    ) -> List[str]:
