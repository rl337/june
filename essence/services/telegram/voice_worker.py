...
async def process_voice_message(msg_data: dict):
    ...
    update = Update(
        update_id=msg_data.get("metadata", {}).get("update_id", 0), message=None
    )
    ...
    chat = Chat(id=int(msg_data["chat_id"]), type="private")
...
async def main():
    ...
    try:
        await create_worker_subscription(
            queue=queue, process_callback=process_voice_message, worker_id=worker_id
        )
