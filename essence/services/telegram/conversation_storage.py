...
_in_memory_storage: Dict[tuple, Dict[str, Any]] = defaultdict(dict)  # (user_id, chat_id) -> metadata dict
...
class ConversationStorage:
    ...
    def set_user_preferences(user_id: str, chat_id: str, name: Optional[str] = None, favorite_color: Optional[str] = None) -> bool:
        ...
        if favorite_color is not None:
            _in_memory_storage[key]['favorite_color'] = favorite_color.strip() if favorite_color else None
        
        logger.info(f"Set user preferences for {user_id}/{chat_id}: name={name}, favorite_color={favorite_color}")
        return True
    ...
    def get_conversation_analytics(user_id: str, chat_id: str) -> Dict[str, Any]:
        ...
        logger.debug(f"PostgreSQL not available - returning empty analytics for {user_id}/{chat_id}")
        return {
    ...
    def get_prompt_template_for_user(user_id: str, name: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        logger.debug(f"Prompt templates not available in MVP (in-memory storage). Returning None for user {user_id}")
        return None
    ...
    def get_prompt_template_for_conversation(user_id: str, chat_id: str, name: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        # For MVP, prompt templates are not stored in-memory
        # Return None to use default prompts
        logger.debug(f"Prompt templates not available in MVP (in-memory storage). Returning None for {user_id}/{chat_id}")
        return None
    ...
    def get_ab_test(test_id: str) -> Optional[Dict[str, Any]]:
        ...
        logger.debug(f"PostgreSQL not available - AB tests not supported for MVP, returning None for test {test_id}")
        return None
    ...
    def update_ab_test(
        ...
        """
        # PostgreSQL not available for MVP - AB tests not supported
        logger.debug(f"PostgreSQL not available - AB tests not supported for MVP, cannot update test {test_id}")
        return False
    ...
    def assign_ab_variant(
        ...
        """
        # PostgreSQL not available for MVP - AB tests not supported
        logger.debug(f"PostgreSQL not available - AB tests not supported for MVP, cannot assign variant for test {test_id}")
        return None
    ...
    def record_ab_metric(
        ...
        """
        # PostgreSQL not available for MVP - AB tests not supported
        logger.debug(f"PostgreSQL not available - AB tests not supported for MVP, cannot record metric for test {test_id}")
        return False
