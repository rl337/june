...
def get_db_connection() -> None:
    ...
    raise RuntimeError(
        "PostgreSQL is not available. Admin auth methods will return defaults."
    )
...
def is_admin(user_id: str) -> bool:
    ...
    logger.debug(
        f"PostgreSQL not available - cannot check admin status for user {user_id}"
    )
    return False
...
def require_admin(user_id: str) -> bool:
    ...
    if not is_admin(user_id):
        raise PermissionError(
            f"User {user_id} is not authorized to perform admin actions"
        )
    return True
