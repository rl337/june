...
class VoiceQualityScorer:
    ...
    def score_voice_message(self, audio_data: bytes, audio_format: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            feedback = self._generate_feedback(analysis, volume_score, clarity_score, noise_score)
            suggestions = self._generate_suggestions(analysis, volume_score, clarity_score, noise_score)
    ...
    def _ensure_wav_format(self, audio_data: bytes, audio_format: Optional[str] = None) -> bytes:
        """
    ...
    def _analyze_with_librosa(self, wav_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            snr_estimate = 20 * np.log10(speech_energy / max(total_energy - speech_energy, 1e-10))
    ...
    def _generate_feedback(self, analysis: Dict[str, Any], volume_score: float, clarity_score: float, noise_score: float) -> str:
        """Generate textual feedback about quality."""
        feedback_parts = []
        
        # Volume feedback
        if volume_score < 30:
            feedback_parts.append("Volume is too quiet - speak closer to the microphone")
        ...
        # Clarity feedback
        if clarity_score < 40:
            feedback_parts.append("Speech clarity is poor - check microphone quality or environment")
        ...
        # Noise feedback
        if noise_score < 30:
            feedback_parts.append("High background noise detected - record in a quieter environment")
        elif noise_score < 50:
    ...
    def _generate_suggestions(self, analysis: Dict[str, Any], volume_score: float, clarity_score: float, noise_score: float) -> List[str]:
        ...
        if not suggestions:
            suggestions.append("Audio quality is good - no specific improvements needed")
