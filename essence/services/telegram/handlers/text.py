    ...
    with tracer.start_as_current_span("telegram.text_message.handle") as span:
        try:
            ...
            if name or favorite_color:
                with tracer.start_as_current_span("telegram.text_message.store_preferences") as pref_span:
                    try:
                        ...
                        pref_span.set_attribute("has_favorite_color", favorite_color is not None)
                        ...
                        pref_span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
            ...
            try:
                ...
                chunk_count = 0
                with tracer.start_as_current_span("telegram.text_message.stream_agent") as stream_span:
                    stream_span.set_attribute("user_id", str(user_id))
                    stream_span.set_attribute("chat_id", str(chat_id))
                    stream_span.set_attribute("message_length", len(user_message) if user_message else 0)
                    ...
                    for message_text, is_final, message_type in stream_agent_message(
                        ...
                        if message_type == "result":
                            ...
                            stream_span.set_attribute("result_length", len(raw_llm_response))
                    ...
                    stream_span.set_attribute("final_response_length", len(raw_llm_response) if raw_llm_response else 0)
                
                # Step 4: Replace with final result when done
                if raw_llm_response:
                    with tracer.start_as_current_span("telegram.text_message.build_and_render") as render_span:
                        render_span.set_attribute("user_id", str(user_id))
                        render_span.set_attribute("chat_id", str(chat_id))
                        render_span.set_attribute("response_length", len(raw_llm_response))
                        try:
                            ...
                            render_span.set_attribute("rendered_parts_count", len(rendered_parts))
                            ...
                            render_span.set_status(trace.Status(trace.StatusCode.ERROR, str(final_error)))
