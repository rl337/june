...
def is_transient_telegram_error(error: Exception) -> bool:
    ...
    if isinstance(error, BadRequest):
        ...
        if any(keyword in error_msg for keyword in ['too large', 'too long', 'invalid', 'format', 'duration']):
            return False
...
async def send_voice_with_error_handling(
    ...
    ogg_file_size = os.path.getsize(ogg_audio_path) if os.path.exists(ogg_audio_path) else 0
    ...
    async def _send_voice():
        ...
        try:
            ...
            if transcript:
                ...
                transcript_preview = transcript[:180] + "..." if len(transcript) > 180 else transcript
    ...
    try:
        ...
        logger.warning("Failed to update status message at start of send_voice_with_error_handling")
    ...
    try:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                try:
                    ...
                    logger.debug("Status message cleanup skipped (may have been deleted already)")
        ...
        logger.error(
            f"Telegram API bad request error (permanent): {e}",
            exc_info=True
        )
        error_message = get_telegram_error_message(e, transcript)
        await status_msg.edit_text(error_message)
        ...
        logger.error(
            f"Telegram API conflict error: {e}",
            exc_info=True
        )
        error_message = get_telegram_error_message(e, transcript)
        await status_msg.edit_text(error_message)
        ...
        # Unexpected errors
        logger.error(
            f"Unexpected error sending voice response: {e}",
            exc_info=True
        )
        await status_msg.edit_text(
            f"❌ **Failed to send voice response:**\n\n"
...
def handle_voice_language_command(transcript: str, user_id: str, chat_id: str) -> tuple[bool, str]:
    """
        ...
        if match:
            ...
            if is_language_supported(language_input):
                ...
                for code, name in supported.items():
                    ...
                    if language_input.lower() in name.lower() or name.lower() in language_input.lower():
                        language_code = code
                        break
            ...
            if language_code:
                ...
                if ConversationStorage.set_language_preference(user_id, chat_id, language_code):
                    ...
                    return True, f"✅ Language changed to **{lang_name}** ({language_code})"
                else:
                    return True, "❌ Failed to change language. Please try again."
            else:
                # Language not found
                lang_list = ", ".join([f"{code} ({name})" for code, name in sorted(supported.items())])
                return True, f"❌ Language '{language_input}' not recognized.\n\nSupported languages: {lang_list}"
...
async def handle_voice_message(
    ...
    if not update.message or not update.message.voice:
        logger.warning(f"Received update without voice message: update_id={update.update_id}")
        return
    ...
    max_file_size = min(
        config.max_file_size if hasattr(config, 'max_file_size') else MAX_AUDIO_SIZE_BYTES,
        MAX_AUDIO_SIZE_BYTES
    )
    ...
    if voice_metadata['file_size'] is not None:
        ...
        logger.debug("Voice message file_size not provided by Telegram, will validate after download")
    ...
    if voice_metadata['duration'] is not None:
        ...
        logger.debug("Voice message duration not provided by Telegram, will validate after processing")
    ...
    if use_queue:
        ...
        try:
            ...
            with tracer.start_as_current_span("voice.download") as span:
                ...
                span.set_attribute("voice.mime_type", voice_metadata.get('mime_type', 'unknown'))
    ...
    try:
        ...
        user_id = str(update.effective_user.id)
        chat_id = str(update.effective_chat.id)
        preferred_language = ConversationStorage.get_language_preference(user_id, chat_id)
        ...
        try:
            ...
            from essence.services.telegram.dependencies.retry import retry_with_exponential_backoff
        ...
        try:
            ...
            logger.debug(f"Using simple prompt without conversation history (gateway removed for MVP)")
            
            # Get custom prompt template if available
            system_prompt = None
            try:
                template = ConversationStorage.get_prompt_template_for_conversation(user_id, chat_id)
                ...
                if template and template.get('template_text'):
                    ...
                    try:
                        ...
                        logger.info(f"Using custom prompt template: {template.get('name')}")
                    except KeyError as e:
                        logger.warning(f"Template variable not found: {e}, using template as-is")
                        system_prompt = template_text
                    except Exception as e:
                        logger.warning(f"Error formatting template: {e}, using template as-is")
                        system_prompt = template_text
            ...
            llm_status = "ok"
            from essence.services.telegram.telegram_utils import stream_llm_response_to_telegram
            ...
            async def call_llm():
                with tracer.start_as_current_span("llm.chat_stream") as span:
                    ...
                    input_length = sum(len(msg.get('content', '')) for msg in chat_messages)
                    ...
                    async with grpc_pool.get_llm_channel() as channel:
                        ...
                        try:
                            ...
                            llm_response, stream_success = await stream_llm_response_to_telegram(
                                message=status_msg,
                                llm_stream=llm_client.chat_stream(chat_messages),
                                prefix="💬 **Response:**\n\n",
                                update_interval=0.1
                            )
                            span.set_attribute("llm.response_length", len(llm_response) if llm_response else 0)
            ...
            try:
                ...
                LLM_GENERATION_DURATION_SECONDS.labels(platform=PLATFORM, status=llm_status).observe(llm_duration)
        ...
        try:
                ...
                with tracer.start_as_current_span("tts.synthesize") as span:
                    ...
                    async with grpc_pool.get_tts_channel() as channel:
                        ...
                        try:
                            ...
                            span.set_attribute("tts.audio_size_bytes", len(tts_audio_bytes))
            ...
            try:
                ...
                TTS_SYNTHESIS_DURATION_SECONDS.labels(platform=PLATFORM, status=tts_status).observe(tts_duration)
            ...
            try:
                ...
                tts_audio_duration = len(tts_audio_bytes) / (16000 * 2) if tts_audio_bytes else 0.0
                # Try to get actual duration using librosa if available
                try:
                    tts_audio_array, tts_sr = librosa.load(io.BytesIO(tts_audio_bytes), sr=16000)
        ...
        try:
            ...
            if ogg_audio_path and os.path.exists(ogg_audio_path):
                try:
                    ...
                    logger.debug(f"Cleaned up temporary file after conversion error: {ogg_audio_path}")
                except Exception as cleanup_error:
                    # Log warning but don't fail the operation
                    logger.warning(f"Failed to delete temp file {ogg_audio_path} after conversion error: {cleanup_error}")
        ...
        try:
            ...
            VOICE_MESSAGES_PROCESSED_TOTAL.labels(platform=PLATFORM, status=status).inc()
            VOICE_PROCESSING_DURATION_SECONDS.labels(platform=PLATFORM, status=status).observe(voice_processing_duration)
    except Exception as e:
        status = "error"
        ...
        try:
            ...
            if 'ogg_audio_path' in locals() and ogg_audio_path and os.path.exists(ogg_audio_path):
                try:
                    os.unlink(ogg_audio_path)
                    logger.debug(f"Cleaned up temporary file in outer exception handler: {ogg_audio_path}")
                except Exception as cleanup_error:
                    logger.warning(f"Failed to delete temp file {ogg_audio_path} in outer exception handler: {cleanup_error}")
        except Exception:
