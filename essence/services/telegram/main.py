...
class TelegramBotService:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.info(
                f"Service configuration loaded. STT address: {self.stt_address}"
            )
    ...
    def _setup_health_endpoint(self):
        ...
        async def queue_status():
            ...
            try:
                ...
                return JSONResponse(
                    content={"error": "Failed to get queue status", "message": str(e)},
                    status_code=500,
                )
    ...
    def _register_handlers(self):
        ...
        async def check_blocked_and_rate_limit(
        ...
        async def voice_wrapper(update, context):
            ...
            await handle_voice_message(
                update, context, self.config, self.stt_address, get_metrics_storage
            )
        ...
        self.application.add_handler(
            CommandHandler("admin_list_blocked", list_blocked_command)
        )
        self.application.add_handler(
            CommandHandler("admin_clear_conversation", clear_conversation_command)
        )
        self.application.add_handler(
            CommandHandler("admin_clear_user", clear_user_conversations_command)
        )
        self.application.add_handler(
            CommandHandler("admin_status", system_status_command)
        )
        self.application.add_handler(
            CommandHandler("admin_message_history", admin_message_history_command)
        )
    ...
    async def _run_async(
        self, use_webhook: bool = False, webhook_url: Optional[str] = None
    ):
        ...
        if use_webhook and webhook_url:
            ...
            try:
                await self.application.bot.set_webhook(
                    url=f"{webhook_url}{webhook_path}", allowed_updates=Update.ALL_TYPES
                )
    ...
    def run(self, use_webhook: bool = False, webhook_url: Optional[str] = None):
        ...
        try:
            asyncio.run(
                self._run_async(use_webhook=use_webhook, webhook_url=webhook_url)
            )
