...
class IntegrationTestService:
    ...
    def _setup_routes(self):
        ...
        async def start_test_run(
            ...
            HTTP_REQUESTS_TOTAL.labels(
                method="POST", endpoint="/tests/run", status_code=200
            ).inc()
            ...
            with self._lock:
                ...
                active_count = sum(
                    1
                    for r in self.test_runs.values()
        ...
        async def get_test_status(run_id: str):
            ...
            HTTP_REQUESTS_TOTAL.labels(
                method="GET", endpoint="/tests/status", status_code=200
            ).inc()
            ...
            if not test_run:
                raise HTTPException(
                    status_code=404, detail=f"Test run {run_id} not found"
                )
        ...
        async def get_test_results(run_id: str):
            ...
            HTTP_REQUESTS_TOTAL.labels(
                method="GET", endpoint="/tests/results", status_code=200
            ).inc()
        ...
        async def get_test_logs(run_id: str, lines: int = 100):
            ...
            HTTP_REQUESTS_TOTAL.labels(
                method="GET", endpoint="/tests/logs", status_code=200
            ).inc()
        ...
        async def list_test_runs(limit: int = 50):
            """List all test runs."""
            HTTP_REQUESTS_TOTAL.labels(
                method="GET", endpoint="/tests/runs", status_code=200
            ).inc()
        ...
        async def cancel_test_run(run_id: str):
            ...
            HTTP_REQUESTS_TOTAL.labels(
                method="DELETE", endpoint="/tests/runs", status_code=200
            ).inc()
    ...
    async def _run_tests(
        self, run_id: str, test_path: Optional[str], test_name: Optional[str]
    ):
        ...
        try:
            ...
            with self._lock:
                ...
                TEST_RUNS_TOTAL.labels(
                    status=status_label, test_path=test_path_label
                ).inc()
                TEST_RUN_DURATION_SECONDS.labels(
                    status=status_label, test_path=test_path_label
                ).observe(duration)
                # Update active test runs gauge
                active_count = sum(
            ...
            logger.info(
                f"Test run {run_id} completed with exit code {exit_code} in {duration:.2f}s"
            )
            ...
            with self._lock:
                ...
                TEST_RUN_DURATION_SECONDS.labels(
                    status="failed", test_path=test_path_label
                ).observe(duration)
                # Update active test runs gauge
                active_count = sum(
                    1
