...
async def instrument_grpc_call(
    ...
    **kwargs
    ...
    try:
        ...
        GRPC_REQUESTS_TOTAL.labels(service=service, method=method, status_code=status_code).inc()
        GRPC_REQUEST_DURATION_SECONDS.labels(service=service, method=method, status_code=status_code).observe(duration)
