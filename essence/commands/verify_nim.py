...
def check_http_health(base_url: str, timeout: int = 10) -> Tuple[bool, str, Dict[str, Any]]:
    """
    ...
    try:
        with httpx.Client(timeout=timeout) as client:
            ...
            if response.status_code == 200:
                return True, f"HTTP health endpoint accessible at {base_url}/health", details
            else:
                return False, f"HTTP health endpoint returned status {response.status_code}", details
    except httpx.TimeoutException:
        return False, f"HTTP health endpoint timeout at {base_url}/health", details
    except httpx.ConnectError:
        return False, f"HTTP health endpoint not reachable at {base_url}/health", details
...
def check_grpc_connectivity(grpc_address: str) -> Tuple[bool, str, Dict[str, Any]]:
    ...
    try:
        ...
        try:
            ...
            channel.close()
            return False, f"gRPC endpoint at {grpc_address} not responding (timeout)", details
        except Exception as e:
            channel.close()
...
def check_grpc_protocol_compatibility(grpc_address: str) -> Tuple[bool, str, Dict[str, Any]]:
    """
    ...
    try:
        ...
        try:
            ...
            return False, "june_grpc_api not available (cannot test protocol compatibility)", details
        ...
        try:
            ...
            details['model_name'] = response.model_name if hasattr(response, 'model_name') else None
            details['max_context_length'] = response.max_context_length if hasattr(response, 'max_context_length') else None
            details['healthy'] = response.healthy if hasattr(response, 'healthy') else None
            
            channel.close()
            return True, f"gRPC protocol compatible - HealthCheck RPC successful", details
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if details['gpu_available']:
                return True, f"GPU available ({details['gpu_count']} GPU(s) detected)", details
            else:
...
class VerifyNIMCommand(Command):
    ...
    def run(self) -> None:
        ...
        critical_checks_passed = (
            self.results['http_health']['success'] and
            self.results['grpc_connectivity']['success']
        )
        
        self.results['ready'] = critical_checks_passed
        self.results['overall_status'] = 'ready' if critical_checks_passed else 'not_ready'
        ...
        if self.results['ready']:
            ...
            if self.args.check_protocol and self.results['grpc_protocol']['success']:
                ...
                print("  ⚠ gRPC protocol compatibility not verified (use --check-protocol).")
        else:
            ...
            if not self.results['grpc_connectivity']['success']:
                print(f"  - gRPC connectivity: {self.results['grpc_connectivity']['message']}")
            if not self.results['gpu_availability']['success']:
                print(f"  - GPU availability: {self.results['gpu_availability']['message']}")
