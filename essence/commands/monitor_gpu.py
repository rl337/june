...
class MonitorGpuCommand(Command):
    ...
    def init(self) -> None:
        ...
        self._metrics["gpu_count"] = Gauge(
            "gpu_count", "Number of GPUs detected", registry=self._registry
        )
    ...
    def run(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Started GPU monitoring metrics server on port {self._metrics_port}"
            )
        except Exception as e:
        ...
        device_count = pynvml.nvmlDeviceGetCount()
        logger.info(
            f"Starting GPU monitoring for {device_count} GPU(s), updating every {self._update_interval}s"
        )
