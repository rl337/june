...
"""
            ...
            return (
                True,
                f"Created model repository structure for '{model_name}' version {version} at {model_dir}",
            )
    ...
    def validate_model_structure(
        ...
        if not tokenizer_files:
            warnings.append(
                "No tokenizer files found (tokenizer.json, vocab.json, etc.)"
            )
        else:
        ...
        if missing_files:
            return (
                False,
                missing_files,
                f"Model structure incomplete. Missing: {', '.join(missing_files)}",
            )
        elif warnings:
            return (
                True,
                [],
                f"Model structure exists but has warnings: {'; '.join(warnings)}",
            )
        else:
    ...
    def list_models(self) -> Tuple[bool, List[Dict[str, any]], str]:
        ...
        if not self.repository_path.exists():
            return (
                False,
                [],
                f"Repository directory does not exist: {self.repository_path}",
            )
...
class SetupTritonRepositoryCommand(Command):
    ...
    def add_args(cls, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--version", "-v", type=str, default="1", help="Model version (default: 1)"
        )
        parser.add_argument(
            "--repository-path",
            type=str,
    ...
    def init(self) -> None:
        ...
        logger.info(
            f"Initialized Triton repository manager (path: {self.args.repository_path})"
        )
    ...
    def run(self) -> None:
        ...
        if action == "create":
            success, message = self.manager.create_model_structure(
                self.args.model, self.args.version
            )
            if success:
                print(f"✅ {message}")
                print(f"\n📁 Model directory created at:")
                print(
                    f"   {self.args.repository_path}/{self.args.model}/{self.args.version}"
                )
                print(f"\n📝 Next steps:")
