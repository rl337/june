...
def check_model_repository(repository_path: str) -> Tuple[bool, str, Dict[str, Any]]:
    ...
    if not repo_path.exists():
        return False, f"Model repository directory does not exist: {repository_path}", details
    
    details['exists'] = True
    
    if not repo_path.is_dir():
        return False, f"Model repository path is not a directory: {repository_path}", details
    ...
    try:
        ...
        for model_dir in repo_path.iterdir():
            if model_dir.is_dir():
                versions = [v.name for v in model_dir.iterdir() if v.is_dir() and v.name.isdigit()]
                if versions:
                    models.append({
                        'name': model_dir.name,
...
class VerifyTensorRTLLMCommand(Command):
    ...
    def run(self) -> None:
        ...
        success, message, details = check_container_connectivity(self.args.tensorrt_llm_url)
        ...
        critical_checks_passed = (
            self.results['container_connectivity']['success'] and
        ...
        if self.results['migration_ready']:
            ...
            print("  Safe to remove inference-api from docker-compose.yml (with legacy profile).")
        else:
            ...
            if not self.results['container_connectivity']['success']:
                print(f"  - Container connectivity: {self.results['container_connectivity']['message']}")
            ...
            if not self.results['model_repository']['success']:
                print(f"  - Model repository: {self.results['model_repository']['message']}")
