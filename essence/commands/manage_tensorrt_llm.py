...
class TensorRTLLMManager:
    ...
    def __init__(self, base_url: str = "http://tensorrt-llm:8002", timeout: int = 300):
        ...
        self.client = httpx.Client(timeout=self.timeout) if HTTP_CLIENT_AVAILABLE else None
    ...
    def load_model(self, model_name: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            error_msg = f"Timeout loading model '{model_name}' (exceeded {self.timeout}s)"
            logger.error(error_msg)
            return False, error_msg
    ...
    def unload_model(self, model_name: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            error_msg = f"Timeout unloading model '{model_name}' (exceeded {self.timeout}s)"
            logger.error(error_msg)
            return False, error_msg
        except httpx.ConnectError as e:
            error_msg = f"Cannot connect to TensorRT-LLM at {self.base_url}: {e}"
    ...
    def list_models(self) -> Tuple[bool, List[Dict[str, any]], str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error_msg = f"Failed to list models: {response.status_code} - {response.text}"
                logger.error(error_msg)
                return False, [], error_msg
            ...
            if response.status_code == 200:
                ...
                if success:
                    ...
                    if model_name in model_names:
                        return True, "UNAVAILABLE", f"Model '{model_name}' exists but is not loaded"
                    else:
                        return False, None, f"Model '{model_name}' not found in repository"
                else:
                    return False, None, f"Cannot check model status: {msg}"
            else:
                return False, None, f"Failed to check model status: {response.status_code} - {response.text}"
...
class ManageTensorRTLLMCommand(Command):
    ...
    def init(self) -> None:
        ...
        self.manager = TensorRTLLMManager(
            base_url=self.args.tensorrt_llm_url,
            timeout=self.args.timeout
        )
        logger.info(f"Initialized TensorRT-LLM manager (URL: {self.args.tensorrt_llm_url})")
