    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            if result["gpu_count"] > 0:
                ...
                result["gpu_memory_allocated_gb"] = torch.cuda.memory_allocated(
                    device
                ) / (1024**3)
                result["gpu_memory_reserved_gb"] = torch.cuda.memory_reserved(
                    device
                ) / (1024**3)
...
def generate_report(
    ...
    if torch_info["torch_available"]:
        report_lines.append(
            f"  PyTorch Version: {torch_info.get('torch_version', 'unknown')}"
        )
        report_lines.append(f"  CUDA Available: {torch_info['cuda_available']}")
        if torch_info["cuda_available"]:
            report_lines.append(
                f"  CUDA Version: {torch_info.get('cuda_version', 'unknown')}"
            )
            report_lines.append(f"  GPU Count: {torch_info['gpu_count']}")
            if torch_info["gpu_count"] > 0:
                report_lines.append(
                    f"  GPU Name: {torch_info.get('gpu_name', 'unknown')}"
                )
                if torch_info.get("gpu_memory_total_gb"):
                    report_lines.append(
                        f"  GPU Memory Total: {torch_info['gpu_memory_total_gb']:.2f} GB"
                    )
                if torch_info.get("gpu_memory_allocated_gb"):
                    report_lines.append(
                        f"  GPU Memory Allocated: {torch_info['gpu_memory_allocated_gb']:.2f} GB"
                    )
                if torch_info.get("gpu_memory_reserved_gb"):
                    report_lines.append(
                        f"  GPU Memory Reserved: {torch_info['gpu_memory_reserved_gb']:.2f} GB"
                    )
                if torch_info.get("gpu_compute_capability"):
                    major, minor = torch_info["gpu_compute_capability"]
                    report_lines.append(
                        f"  GPU Compute Capability: {major}.{minor} (sm_{major}{minor})"
                    )
    report_lines.append("")
    ...
    report_lines.append(
        f"  BitsAndBytes Available: {bnb_info['bitsandbytes_available']}"
    )
    if bnb_info["bitsandbytes_available"]:
        report_lines.append(
            f"  BitsAndBytes Version: {bnb_info.get('bitsandbytes_version', 'unknown')}"
        )
    report_lines.append("")
    ...
    if model_info["model_loaded"]:
        report_lines.append(
            f"  Model Device: {model_info.get('model_device', 'unknown')}"
        )
        report_lines.append(
            f"  Model Dtype: {model_info.get('model_dtype', 'unknown')}"
        )
        report_lines.append(
            f"  Quantization Detected: {model_info['quantization_detected']}"
        )
        if model_info["quantization_detected"]:
            report_lines.append(
                f"  Quantization Type: {model_info.get('quantization_type', 'unknown')}"
            )
        report_lines.append(
            f"  Strategy Use Quantization: {model_info.get('strategy_use_quantization', 'unknown')}"
        )
        report_lines.append(
            f"  Strategy Quantization Bits: {model_info.get('strategy_quantization_bits', 'unknown')}"
        )
        report_lines.append(
            f"  Strategy Device: {model_info.get('strategy_device', 'unknown')}"
        )
    else:
        report_lines.append(
            f"  Strategy Use Quantization: {model_info.get('strategy_use_quantization', 'unknown')}"
    ...
    if not torch_info["cuda_available"]:
        ...
        report_lines.append(
            "  ⚠️  BitsAndBytes not available - quantization will not work"
        )
        ...
        if model_info["model_loaded"]:
            if model_info["quantization_detected"]:
                ...
                if (
                    expected_bits in detected_bits
                    or str(quantization_bits) in detected_bits
                ):
                    report_lines.append(
                        f"  ✅ Quantization configured and active: {quantization_bits}-bit"
                    )
                else:
                    report_lines.append(
                        f"  ⚠️  Quantization configured ({quantization_bits}-bit) but detected type: {model_info.get('quantization_type', 'unknown')}"
                    )
            else:
                report_lines.append(
                    f"  ⚠️  Quantization configured ({quantization_bits}-bit) but not detected in loaded model"
                )
        else:
            report_lines.append(
                f"  ℹ️  Quantization configured ({quantization_bits}-bit) - model not loaded yet"
            )
    elif not quantization_configured:
        report_lines.append(
            "  ℹ️  Quantization not configured (USE_QUANTIZATION=false)"
        )
    else:
        report_lines.append(
            f"  ⚠️  Invalid quantization bits: {quantization_bits} (expected 4 or 8)"
        )
