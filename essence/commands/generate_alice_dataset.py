...
class GenerateAliceDatasetCommand(Command):
    ...
    def add_args(cls, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--output-dir",
            type=Path,
            default=Path(os.getenv("JUNE_DATA_DIR", "/home/rlee/june_data")) / "datasets" / "alice_in_wonderland",
    ...
    def run(self) -> None:
        ...
        if len(passages) < self.args.num_passages:
            logger.warning(f"Only generated {len(passages)} passages (requested {self.args.num_passages})")
