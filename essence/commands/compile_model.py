...
def check_tensorrt_llm_build_tools() -> Tuple[bool, str, Dict[str, Any]]:
    ...
    try:
        result = subprocess.run(
            ["which", "trtllm-build"], capture_output=True, text=True, timeout=5
        )
    ...
    if details["tools_available"]:
        ...
        return (
            False,
            "TensorRT-LLM build tools not found in PATH (may need to use Docker container)",
            details,
        )
    else:
...
def check_model_repository_structure(
    model_name: str, repository_path: str = "/home/rlee/models/triton-repository"
    ...
    is_valid, missing_files, message = manager.validate_model_structure(
...
def check_model_files(
    ...
    tokenizer_files = list(model_dir.glob("tokenizer*.json")) + list(
        model_dir.glob("vocab*.json")
    )
    if tokenizer_files:
        details["has_tokenizer"] = True
        details["files_found"].extend([f.name for f in tokenizer_files])
    else:
        details["missing_files"].append(
            "tokenizer files (tokenizer*.json or vocab*.json)"
        )

    # Model is ready for loading if all critical files are present
    details["ready_for_loading"] = (
        details["has_config"]
        and details["has_engine_files"]
        and details["has_tokenizer"]
    )
    ...
    if details["ready_for_loading"]:
        ...
        return (
            True,
            f"Model appears to be compiled (found config.pbtxt and engine files, but missing tokenizer files)",
            details,
        )
    elif details["has_config"]:
        return (
            False,
            "Model directory exists but missing engine files (compilation incomplete)",
            details,
        )
    else:
        return (
            False,
            "Model directory exists but missing required files (needs compilation)",
            details,
        )
...
def check_model_readiness(
    ...
    details["checks"]["directory"] = {
        "success": True,
        "message": f"Model directory exists: {model_dir}",
    }
    ...
    if not config_file.exists():
        details["checks"]["config"] = {
            "success": False,
            "message": "config.pbtxt not found",
        }
        ...
        try:
            ...
            if len(config_content.strip()) == 0:
                details["checks"]["config"] = {
                    "success": False,
                    "message": "config.pbtxt is empty",
                }
                details["issues"].append("config.pbtxt is empty")
            elif "name:" not in config_content or "platform:" not in config_content:
                details["checks"]["config"] = {
                    "success": False,
                    "message": "config.pbtxt appears invalid (missing required fields)",
                }
                details["issues"].append("config.pbtxt appears invalid")
            else:
                details["checks"]["config"] = {
                    "success": True,
                    "message": "config.pbtxt exists and appears valid",
                }
        except Exception as e:
            details["checks"]["config"] = {
                "success": False,
                "message": f"Error reading config.pbtxt: {e}",
            }
    ...
    engine_files = list(model_dir.glob("*.engine"))
    if not engine_files:
        details["checks"]["engine_files"] = {
            "success": False,
            "message": "No .engine files found",
        }
        details["issues"].append("Missing TensorRT-LLM engine files")
    else:
        details["checks"]["engine_files"] = {
            "success": True,
            "message": f"Found {len(engine_files)} engine file(s)",
        }
    ...
    if not tokenizer_files:
        details["checks"]["tokenizer"] = {
            "success": False,
            "message": "No tokenizer files found",
        }
        details["issues"].append("Missing tokenizer files")
    else:
        details["checks"]["tokenizer"] = {
            "success": True,
            "message": f"Found {len(tokenizer_files)} tokenizer file(s)",
        }

    # Overall readiness
    all_checks_passed = all(
        check.get("success", False) for check in details["checks"].values()
    )
    ...
    if all_checks_passed:
        return (
            True,
            "✅ Model is ready for loading! All required files are present and valid.",
            details,
        )
    else:
        return (
            False,
            f"❌ Model is not ready for loading. {len(details['issues'])} issue(s) found.",
            details,
        )
...
def check_tokenizer_files(
    model_hf_name: str, model_cache_dir: str = "/home/rlee/models"
    ...
    model_dir_pattern = (
        ...
        / "huggingface"
        / "hub"
        / f"models--{model_hf_name.replace('/', '--')}"
    )
...
def generate_tokenizer_copy_commands(
    ...
    files_exist, msg, tokenizer_files = check_tokenizer_files(
        model_hf_name, model_cache_dir
    )
...
"""
    ...
    commands.append(
        f"rsync -av '{model_dir_pattern}/'tokenizer* '{model_dir_pattern}/'vocab* '{target_dir}/'"
    )
...
def generate_config_pbtxt(
    model_name: str, max_batch_size: int = 0, max_input_len: int = 131072
) -> str:
    """
...
class CompileModelCommand(Command):
    ...
    def run(self) -> None:
        ...
        results = {"model_name": model_name, "checks": {}, "ready": False, "issues": []}
        ...
        if self.args.generate_template:
            template = generate_compilation_template(
                model_name, self.args.model_hf_name
            )
        ...
        if self.args.generate_config:
            ...
            config_path = (
                Path(self.args.repository_path) / model_name / "1" / "config.pbtxt"
            )
            ...
            if not self.args.json:
                ...
                if config_path.parent.exists():
                    try:
                        ...
                        print(
                            "   Please save the config.pbtxt content manually to the model directory"
                        )
                else:
        ...
        if self.args.check_readiness:
            ...
            ready, msg, readiness_details = check_model_readiness(
                model_name, self.args.repository_path
            )
            results["readiness"] = {
                "ready": ready,
            ...
            if not self.args.json:
                ...
                for check_name, check_result in readiness_details["checks"].items():
                    ...
                    print(
                        f"{status} {check_name.replace('_', ' ').title()}: {check_result.get('message', '')}"
                    )
                ...
                if not ready and readiness_details.get("issues"):
                    ...
                    if (
                        not readiness_details["checks"]
                        .get("engine_files", {})
                        .get("success")
                    ):
                        print(
                            "  2. Compile the model using TensorRT-LLM build tools: --generate-template"
                        )
                    if (
                        not readiness_details["checks"]
                        .get("tokenizer", {})
                        ...
                        print(
                            "  3. Copy tokenizer files: --generate-tokenizer-commands"
                        )
                elif ready:
                    print("\n🚀 Ready to load! Use:")
                    print(
        ...
        if (
            not self.args.check_prerequisites
            and not self.args.generate_template
            and not self.args.generate_config
            and not self.args.generate_tokenizer_commands
            and not self.args.check_readiness
        ):
            ...
            print(
                "Use --generate-tokenizer-commands to get tokenizer file copy commands"
            )
