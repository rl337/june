...
AUDIO_PROCESSING_TIME = Histogram(
    "stt_audio_processing_seconds", "Audio processing time"
)
...
class PassThroughSTTService(asr_pb2_grpc.SpeechRecognitionServicer):
    ...
    async def RecognizeStream(
        self,
        request_iterator: AsyncGenerator[RecognitionRequest, None],
            ...
            try:
                ...
                async for request in request_iterator:
                    ...
                    if request.audio_data:
                        ...
                        chunk_duration = len(request.audio_data) / (
                            16000 * 2
                        )  # 2 bytes per sample
                ...
                error_result = RecognitionResult(
                    transcript="", confidence=0.0, is_final=True, timestamp_us=0
                )
                yield error_result
