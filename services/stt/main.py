...
REQUEST_COUNT = Counter(
    "stt_requests_total", "Total requests", ["method", "status"], registry=REGISTRY
)
REQUEST_DURATION = Histogram(
    "stt_request_duration_seconds", "Request duration", registry=REGISTRY
)
AUDIO_PROCESSING_TIME = Histogram(
    "stt_audio_processing_seconds", "Audio processing time", registry=REGISTRY
)
VAD_DETECTIONS = Counter(
    "stt_vad_detections_total", "VAD detections", ["action"], registry=REGISTRY
)
MODEL_LOAD_TIME = Histogram(
    "stt_model_load_seconds", "Model loading time", registry=REGISTRY
)
TRANSCRIPTION_TIME = Histogram(
    "stt_transcription_time_seconds", "Transcription time", registry=REGISTRY
)
ACTIVE_CONNECTIONS = Gauge(
    "stt_active_connections", "Active gRPC connections", registry=REGISTRY
)
ERROR_COUNT = Counter(
    "stt_errors_total", "Total errors", ["error_type"], registry=REGISTRY
)
...
class STTService(asr_pb2_grpc.SpeechToTextServicer):
    ...
    async def RecognizeStream(
        self,
        request_iterator: AsyncGenerator[AudioChunk, None],
            ...
            with Timer("recognition_stream"):
                ...
                async for chunk in request_iterator:
                    ...
                    if len(audio_buffer) > self.sample_rate * 5:  # 5 seconds
                        interim_result = await self._transcribe_audio(
                            audio_buffer, is_final=False
                        )
                        if interim_result:
                            if span:
                                span.set_attribute(
                                    "stt.interim_transcript_length",
                                    len(interim_result.transcript),
                                )
                ...
                # Final transcription
                if audio_buffer:
                    final_result = await self._transcribe_audio(
                        audio_buffer, is_final=True
                    )
                    if final_result:
                        if span:
                            span.set_attribute(
                                "stt.transcript_length", len(final_result.transcript)
                            )
                            span.set_attribute(
                                "stt.confidence", final_result.confidence
                            )
                            span.set_attribute(
                                "stt.detected_language",
                                final_result.detected_language or "unknown",
                            )
                            span.set_attribute("stt.chunk_count", chunk_count)
                            span.set_attribute(
                                "stt.total_audio_size_bytes", total_audio_size
                            )
                        yield final_result
                    elif span:
                        span.set_status(
                            trace.Status(
                                trace.StatusCode.ERROR, "Empty transcription result"
                            )
                        )
    ...
    async def Recognize(
        self, request: RecognitionRequest, context: grpc.aio.ServicerContext
    ) -> RecognitionResponse:
        ...
        if tracer is not None:
            ...
            if (
                request.config
                and hasattr(request.config, "language")
                and request.config.language
            ):
        ...
        try:
                ...
                if VALIDATION_AVAILABLE and input_validator:
                    try:
                        ...
                        if (
                            ...
                            > input_validator.MAX_AUDIO_FILE_SIZE
                        ):
                            context.set_code(grpc.StatusCode.INVALID_ARGUMENT)
                            context.set_details(
                                f"Audio file size ({len(request.audio_data)} bytes) exceeds maximum allowed size ({input_validator.MAX_AUDIO_FILE_SIZE} bytes)"
                            )
                        ...
                        if request.sample_rate <= 0 or request.sample_rate > 48000:
                            context.set_code(grpc.StatusCode.INVALID_ARGUMENT)
                            context.set_details(
                                f"Invalid sample rate: {request.sample_rate}. Must be between 1 and 48000"
                            )
                            return RecognitionResponse()
            ...
            try:
                # Process audio data
                audio_data = await self._process_audio_data(
                    request.audio_data, request.sample_rate
                )

                # Calculate audio duration
                audio_duration = (
                    len(audio_data) / request.sample_rate
                ...
                result = await self._transcribe_audio(
                    audio_data, is_final=True, language=language
                )
                ...
                if result:
                    ...
                    return RecognitionResponse(
                        results=[result], processing_time_ms=processing_time_ms
                    )
                else:
    ...
    async def HealthCheck(
        ...
        return HealthResponse(
            healthy=is_healthy, version="0.2.0", model_name=config.stt.model_name
        )
        ...
        try:
            ...
            if chunk.sample_rate != self.sample_rate:
                audio_data = await self._resample_audio(
                    audio_data, chunk.sample_rate, self.sample_rate
                )
    ...
    async def _transcribe_audio(
        ...
        try:
            if (
                not audio_data or len(audio_data) < self.sample_rate * 0.1
            ):  # Less than 100ms
                return None
            ...
            # Calculate timing
            start_time_us = (
                int(result["segments"][0]["start"] * 1_000_000)
                if result["segments"]
                else 0
            )
            end_time_us = (
                int(result["segments"][-1]["end"] * 1_000_000)
                if result["segments"]
                else len(audio_array) / self.sample_rate * 1_000_000
            )
            ...
            return RecognitionResult(
                ...
                or "",  # ISO 639-1 code of detected language
            )
    ...
    async def _detect_voice_activity(self, audio_data: List[float]) -> bool:
        ...
        try:
            if (
                not self.vad or len(audio_data) < self.sample_rate * 0.02
            ):  # Less than 20ms
                return False
...
async def serve():
    ...
    if RATE_LIMIT_AVAILABLE:
        ...
        logger.info(
            "Rate limiting enabled for STT service (in-memory, Redis not required)"
        )
