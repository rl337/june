...
def stream_chat_response_agent(
    ...
    try:
        ...
        for line, is_final_line in streaming_popen_generator(
                ...
                if message:
                    ...
                    if is_result:
                        ...
                        if accumulated_message:
                            ...
                            if appended_normalized == result_normalized:
                                logger.info(
                                    f"✅ Chunk appending validated: appended chunks match result ({len(accumulated_message)} chars)"
                                )
                            else:
                        ...
                        logger.info(
                            f"Replaced with result message (full accumulated): {len(message)} chars"
                        )
                    else:
                    ...
                    if message_updated:
                        ...
                        if (
                            ...
                            if is_result:
                                logger.info(
                                    f"Yielding result message (authoritative): length={len(accumulated_message)} chars"
                                )
                            else:
                                logger.info(
            ...
            if is_final_line:
                logger.info(
                    f"Received final line from generator. pending_messages={len(pending_messages)}, seen_messages={len(seen_messages)}"
                )
                ...
                if isinstance(content, list):
                    for item in content:
                        if isinstance(item, dict) and item.get("type") == "text":
                            ...
                            if (
                                text and len(text) > 5
                            ):  # Lower threshold for earlier messages
