...
def simulate_handler_logic(message_text, raw_llm_response, first_message_pattern=None, update_first_pattern=True):
    ...
    if not raw_llm_response:
        return message_text, True, message_text[:30] if len(message_text) >= 30 else message_text
    
    # Update first_message_pattern from the first message we see
    if update_first_pattern and not first_message_pattern:
        first_message_pattern = raw_llm_response[:30] if len(raw_llm_response) >= 30 else raw_llm_response
...
def simulate_streaming_flow(json_lines):
    ...
    first_chunk_start = assistant_chunks[0][:20] if len(assistant_chunks[0]) >= 20 else assistant_chunks[0]
    ...
    for chunk in assistant_chunks[1:]:
        ...
        if accumulated in chunk:
            ...
            accumulated = accumulated + chunk  # Potential bug: might append when should replace
...
def test_e2e_streaming_no_duplication_in_final(test_file):
    ...
    length_ratio = len(final_raw_llm_response) / len(expected_result) if expected_result else 1.0
...
def test_e2e_streaming_result_overwrites_duplicates():
    ...
    assert final_raw_llm_response.count('# Header 1') == 1, "Final message should not contain duplication"
