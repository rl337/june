...
async def handle_voice_message(
    ...
    if use_queue:
        ...
        try:
            from voice_queue import VoiceMessageQueue
    ...
    try:
        ...
        try:
            ...
            from dependencies.retry import retry_with_exponential_backoff
        ...
        try:
            ...
            from telegram_utils import stream_llm_response_to_telegram
                ...
                async with httpx.AsyncClient(timeout=5.0) as client:
                    ...
                    try:
                        ...
                        if conv_response.status_code == 200:
                            ...
                            metadata['language_preference'] = preferred_language
                            ...
                            if update_response.status_code in (200, 204):
                                logger.info(f"Updated conversation metadata with detected language: {detected_language}")
                            else:
                                logger.warning(f"Failed to update conversation metadata: {update_response.status_code}")
                        else:
                            logger.warning(f"Failed to get conversation for metadata update: {conv_response.status_code}")
                    except Exception as metadata_error:
                        logger.warning(f"Failed to update conversation metadata with detected language: {metadata_error}")
        ...
        try:
            ...
            async def call_tts():
                async with grpc_pool.get_tts_channel() as channel:
                    ...
                    return await tts_client.synthesize(
                        llm_response,
                        voice_id="default",
            ...
            try:
                ...
                async with httpx.AsyncClient(timeout=5.0) as client:
                    ...
                    try:
                        ...
                        if conv_response.status_code == 200:
                            conversation = conv_response.json()
                            metadata_dict = conversation.get('metadata', {})
                            if not isinstance(metadata_dict, dict):
                                metadata_dict = {}
                            metadata_dict['detected_language'] = detected_language
                            metadata_dict['language_preference'] = preferred_language
