"""Helper module for calling TelegramResponse agent and formatting responses."""
import json
import logging
import subprocess
import os
...
def call_telegram_response_agent(
    user_message: str, 
    ...
    """
    Call the TelegramResponse agent with a user message.
    ...
    if agenticness_dir is None:
        # Default to ../agenticness from telegram service directory
    ...
    if user_id is not None and chat_id is not None:
        agent_script = os.path.join(agenticness_dir, "scripts", "telegram_response_agent.sh")
    else:
        logger.warning("user_id and chat_id not provided - using simple script without session support")
        agent_script = os.path.join(agenticness_dir, "scripts", "telegram_response_agent_simple.sh")
    
    if not os.path.exists(agent_script):
        logger.error(f"Agent script not found: {agent_script}")
        return {
            "error": "Agent script not found",
            "message": "The TelegramResponse agent is not available."
        }
    ...
    try:
        ...
        env["AGENT_ID"] = env.get("AGENT_ID", "telegram-response-agent")
        ...
        logger.info(f"Calling TelegramResponse agent with message: {user_message[:50]}...")
        import pty
        import select
...
def stream_telegram_response_agent(
    user_message: str,
    agenticness_dir: Optional[str] = None,
    user_id: Optional[int] = None,
    chat_id: Optional[int] = None,
    ...
    """
    Stream responses from the TelegramResponse agent as they arrive.
    
    Uses PTY (pseudo-terminal) to stream output line-by-line, parsing JSON as it arrives
    ...
    if not os.path.exists(agent_script):
        logger.error(f"Agent script not found: {agent_script}")
        yield ("⚠️ The TelegramResponse agent is not available.", True)
        return
    
    if not os.access(agent_script, os.X_OK):
        logger.error(f"Agent script is not executable: {agent_script}")
        yield ("⚠️ The TelegramResponse agent script is not executable.", True)
        return
    ...
    try:
        ...
        while True:
            ...
            # Check line timeout (no JSON line received recently, not just no activity)
            # Only timeout if we haven't received a JSON line in a while
            ...
            if time_since_last_json > line_timeout and last_json_line_time > start_time:
                logger.warning(f"No JSON line received for {time_since_last_json}s, timing out")
                try:
                    os.kill(process_pid, 15)  # SIGTERM
                except:
                    pass
            ...
            # Read from PTY (non-blocking with shorter timeout for more responsive reading)
            ready, _, _ = select.select([process_fd], [], [], 0.05)  # Shorter timeout for more frequent checks
            if ready:
                try:
                    # Read smaller chunks more frequently for better streaming
                    data = os.read(process_fd, 1024)
                    if data:
        ...
        # Restore PTY settings and close
        try:
            if 'old_settings' in locals():
                termios.tcsetattr(process_fd, termios.TCSADRAIN, old_settings)
            os.close(process_fd)
        except:
            pass
...
def _extract_human_readable_from_json_line(line: str) -> Optional[str]:
    ...
    actual response text that should be sent to Telegram.
    
    Args:
        line: A single line that should be a JSON object
    
    Returns:
        Human-readable text to send, or None if no valid text found
    """
    try:
        json_obj = json.loads(line)
...
def format_agent_response_for_telegram(response_data: Dict[str, Any]) -> str:
    """
    Format agent response data into human-readable Telegram message.
    
    Args:
        response_data: Dictionary from agent response
    
    Returns:
        Formatted message string for Telegram (max 4096 characters for Telegram)
    """
