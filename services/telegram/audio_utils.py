...
def reduce_noise(audio_data: bytes, reduction_strength: float = 0.5) -> bytes:
    ...
    try:
        ...
        audio_array, sample_rate = librosa.load(io.BytesIO(audio_data), sr=16000, mono=True)
...
def enhance_audio_for_stt(
    ...
    if enable_noise_reduction:
        audio_data = reduce_noise(audio_data, reduction_strength=noise_reduction_strength)
...
def compress_audio_for_telegram(
    ...
    if preset and preset in COMPRESSION_PRESETS:
        ...
        logger.info(f"Using compression preset '{preset}': {bitrate} bps ({COMPRESSION_PRESETS[preset]['description']})")
    ...
    original_size_estimate = int(audio.frame_rate * audio.channels * 2 * duration_seconds)  # 16-bit = 2 bytes/sample
    
    # Estimate compressed size (OGG/OPUS at target bitrate)
    compressed_size_estimate = int((bitrate / 8) * duration_seconds)  # bitrate in bits, convert to bytes
    ...
    try:
        ...
        compressed_audio.export(
            ...
            parameters=[
                ...
                "-ar", "48000",  # 48kHz sample rate (Telegram recommended)
            ]
        )
        ...
        if os.path.exists(output_path):
            ...
            compression_info["compression_ratio"] = compression_info["original_size"] / max(actual_size, 1)
...
def find_optimal_compression(
    ...
    for preset_name, preset_config in COMPRESSION_PRESETS.items():
        ...
        if estimated_size > max_file_size:
            logger.debug(f"Preset '{preset_name}' ({bitrate} bps) would exceed max_file_size, skipping")
            continue
        ...
        if quality_score < quality_threshold:
            logger.debug(f"Preset '{preset_name}' quality score {quality_score:.2f} below threshold {quality_threshold}, skipping")
            continue
        ...
        size_efficiency = 1.0 - (estimated_size / max_file_size)  # How much room we have
    ...
    if best_preset is None:
        ...
        best_info = {
            ...
            "estimated_size": int((COMPRESSION_PRESETS["aggressive"]["bitrate"] / 8) * duration_seconds),
            "quality_score": COMPRESSION_PRESETS["aggressive"]["bitrate"] / TELEGRAM_MAX_BITRATE,
            "size_efficiency": 1.0,
            "overall_score": 0.0
        }
    
    logger.info(f"Selected optimal compression preset: '{best_preset}' (score: {best_score:.2f})")
