...
class AudioTestSuite:
    ...
    def _download_librispeech(self):
        ...
        if not (librispeech_dir / "LibriSpeech" / "test-clean").exists():
            ...
            subprocess.run(
                ["tar", "-xzf", str(tar_file), "-C", str(librispeech_dir)], check=True
            )
    ...
    def _download_ljspeech(self):
        ...
        if not (ljspeech_dir / "LJSpeech-1.1").exists():
            ...
            subprocess.run(
                ["tar", "-xjf", str(tar_file), "-C", str(ljspeech_dir)], check=True
            )
    ...
    def load_stt_samples(
        self, dataset: str = "librispeech_test", max_samples: int = 50
    ) -> List[AudioSample]:
        ...
        if dataset == "librispeech_test":
            librispeech_dir = (
                self.test_data_dir / "librispeech" / "LibriSpeech" / "test-clean"
            )
    ...
    def load_tts_samples(
        self, dataset: str = "ljspeech_test", max_samples: int = 30
    ) -> List[str]:
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                ...
                if sample.file_path.startswith("synthetic"):
                    ...
                    transcribed_text = (
                        sample.text
                    )  # Perfect transcription for synthetic
                else:
    ...
    def generate_report(
        ...
        print(
            f"  Average Processing Time: {stt_metrics.get('average_processing_time', 0):.3f}s"
        )
        print(
            f"  Successful Samples: {stt_metrics.get('successful_samples', 0)}/{stt_metrics.get('total_samples', 0)}"
        )

        print(f"\nTTS Performance:")
        print(f"  Average Duration: {tts_metrics.get('average_duration', 0):.3f}s")
        print(
            f"  Average Processing Time: {tts_metrics.get('average_processing_time', 0):.3f}s"
        )
        print(
            f"  Successful Samples: {tts_metrics.get('successful_samples', 0)}/{tts_metrics.get('total_samples', 0)}"
        )
...
def main():
    ...
    parser.add_argument(
        "--download", action="store_true", help="Download test datasets"
    )
    ...
    parser.add_argument(
        "--data-dir", default="/data/test_audio", help="Test data directory"
    )
    parser.add_argument(
        "--max-samples", type=int, default=50, help="Maximum samples per test"
    )
