...
class AudioServiceTester:
    ...
    def generate_report(
        self, stt_results: Dict[str, Any], tts_results: Dict[str, Any]
    ) -> str:
        ...
        report = {
            ...
            "summary": {
                "stt_success_rate": stt_results["successful_tests"]
                / stt_results["test_cases"]
                if stt_results["test_cases"] > 0
                else 0,
                "tts_success_rate": tts_results["successful_tests"]
                / tts_results["test_cases"]
                if tts_results["test_cases"] > 0
                else 0,
                "overall_success": (
                    stt_results["successful_tests"] + tts_results["successful_tests"]
                )
                / (stt_results["test_cases"] + tts_results["test_cases"])
                if (stt_results["test_cases"] + tts_results["test_cases"]) > 0
                else 0,
            },
        }
...
def main():
    ...
    parser.add_argument(
        "--data-dir", default="/tmp/audio_tests", help="Test data directory"
    )
    ...
    if stt_results or tts_results:
        ...
        if stt_results:
            ...
            print(
                f"  Successful Tests: {stt_results['successful_tests']}/{stt_results['test_cases']}"
            )
            print(
                f"  Average Response Time: {stt_results['average_response_time']:.3f}s"
            )
        ...
        if tts_results:
            ...
            print(
                f"  Successful Tests: {tts_results['successful_tests']}/{tts_results['test_cases']}"
            )
            print(
                f"  Average Response Time: {tts_results['average_response_time']:.3f}s"
            )
