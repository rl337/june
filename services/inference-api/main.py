...
try:
    ...
    logger.warning(
        "june-rate-limit package not available, rate limiting disabled for gRPC"
    )
...
REQUEST_DURATION = Histogram(
    "inference_request_duration_seconds", "Request duration", registry=REGISTRY
)
TOKEN_COUNT = Counter(
    "inference_tokens_total", "Total tokens generated", ["model"], registry=REGISTRY
)
TOKEN_GENERATION_RATE = Histogram(
    "inference_token_generation_rate",
    "Token generation rate (tokens/second)",
    registry=REGISTRY,
)
MODEL_LOAD_TIME = Histogram(
    "inference_model_load_seconds", "Model loading time", registry=REGISTRY
)
RAG_RETRIEVAL_TIME = Histogram(
    "inference_rag_retrieval_seconds", "RAG retrieval time", registry=REGISTRY
)
CONTEXT_USAGE = Gauge(
    "inference_context_usage_tokens", "Context usage in tokens", registry=REGISTRY
)
ACTIVE_CONNECTIONS = Gauge(
    "inference_active_connections", "Active gRPC connections", registry=REGISTRY
)
ERROR_COUNT = Counter(
    "inference_errors_total", "Total errors", ["error_type"], registry=REGISTRY
)
...
class InferenceAPIService(llm_pb2_grpc.LLMInferenceServicer):
    ...
    async def GenerateStream(
        self, request: GenerationRequest, context: grpc.aio.ServicerContext
        ...
        try:
            with Timer("generation_stream"):
                ...
                if VALIDATION_AVAILABLE and input_validator:
                    try:
                        ...
                        if span:
                            span.set_status(
                                trace.Status(
                                    trace.StatusCode.ERROR, f"Invalid prompt: {str(e)}"
                                )
                            )
                        ...
                        error_chunk = GenerationChunk(
                            token="", is_final=True, finish_reason=FinishReason.ERROR
                        )
                        yield error_chunk
                        return
                ...
                async for chunk in self._generate_tokens_stream(
                    request.prompt, request.params, context_data
                ):
                    chunk_count += 1
                    yield chunk
        ...
        try:
            with Timer("generation"):
                ...
                if span:
                    ...
                    span.set_attribute(
                        "llm.completion_tokens", usage_stats.completion_tokens
                    )
    ...
    async def ChatStream(
        self, request: ChatRequest, context: grpc.aio.ServicerContext
        ...
        try:
            with Timer("chat_stream"):
                        ...
                        if span:
                            span.set_status(
                                trace.Status(
                                    ...
                                    f"Invalid message content: {str(e)}",
                                )
                            )
                            span.record_exception(e)
                        context.set_code(grpc.StatusCode.INVALID_ARGUMENT)
                ...
                # Format conversation
                formatted_prompt = await self._format_conversation(
                    request.messages, context_data
                )

                if span:
                    span.set_attribute(
                        "llm.formatted_prompt_length", len(formatted_prompt)
                    )

                # Generate response
                chunk_count = 0
                async for chunk in self._generate_tokens_stream(
                    formatted_prompt, request.params, context_data
                ):
                    chunk_count += 1
    ...
    async def Embed(
        self, request: EmbeddingRequest, context: grpc.aio.ServicerContext
    ) -> EmbeddingResponse:
    ...
    async def HealthCheck(
        self, request: HealthRequest, context: grpc.aio.ServicerContext
    ) -> HealthResponse:
    ...
    async def _format_conversation(
        self, messages: List[ChatMessage], context_data: Dict[str, Any]
    ) -> str:
    ...
    async def _generate_tokens_stream(
        self, prompt: str, params: GenerationParameters, context_data: Dict[str, Any]
    ) -> AsyncGenerator[GenerationChunk, None]:
    ...
    async def _generate_text(
        ...
        try:
            ...
            if "out of memory" in error_msg or "oom" in error_msg:
                ...
                raise RuntimeError(
                    f"Out of memory error. Try reducing max_tokens or input length."
                ) from e
            elif "timeout" in error_msg or "timed out" in error_msg:
                ERROR_COUNT.labels(error_type="timeout").inc()
                logger.error(f"Inference timeout: {e}")
                raise RuntimeError(
                    f"Inference timed out. Try reducing max_tokens or increasing timeout."
                ) from e
            else:
    ...
    async def _load_models(self):
        ...
        with Timer("model_loading"):
            ...
            if (
                self.llm_strategy is not None
                and hasattr(self.llm_strategy, "_model")
                and self.llm_strategy._model is not None
                and hasattr(self.llm_strategy, "_tokenizer")
                and self.llm_strategy._tokenizer is not None
            ):
                logger.info("LLM model already loaded. Skipping reload.")
            else:
                logger.info(
                    f"Loading LLM model using Qwen3LlmStrategy: {config.model.name}"
                )
                ...
                self.llm_strategy = Qwen3LlmStrategy(
                    ...
                    if hasattr(config.model, "use_yarn")
                    else None,
                    huggingface_token=config.model.huggingface_token
                    if hasattr(config.model, "huggingface_token")
                    else None,
                    model_cache_dir=config.model.model_cache_dir
                    if hasattr(config.model, "model_cache_dir")
                    else None,
                    local_files_only=config.model.local_files_only
                    if hasattr(config.model, "local_files_only")
                    else None,
                    use_quantization=config.model.use_quantization
                    if hasattr(config.model, "use_quantization")
                    else None,
                    quantization_bits=config.model.quantization_bits
                    if hasattr(config.model, "quantization_bits")
                    else None,
                )
        ...
        try:
            ...
            if db_url and db_url.strip() and db_url.strip() != "":
                try:
                    ...
                    logger.warning(
                        f"Database connection failed (optional): {e}. Continuing without database."
                    )
                    self.db_engine = None
            else:
                logger.debug(
                    "POSTGRES_URL not configured, skipping database connection (not required for MVP)"
                )
                self.db_engine = None
            ...
            if config.nats.url:
                try:
                    ...
                    logger.warning(
                        f"NATS connection failed (optional): {e}. Continuing without NATS."
                    )
                    self.nats_client = None
            else:
                logger.debug(
                    "NATS_URL not configured, skipping NATS connection (not required for MVP)"
                )
                self.nats_client = None
    ...
    async def _check_model_health(self) -> bool:
        ...
        try:
            ...
            if (
                not hasattr(self.llm_strategy, "_model")
                or self.llm_strategy._model is None
            ):
                return False
            if (
                not hasattr(self.llm_strategy, "_tokenizer")
                or self.llm_strategy._tokenizer is None
            ):
                return False
    ...
    async def _check_gpu_health(self) -> bool:
        ...
        try:
            ...
            if not hasattr(self.llm_strategy, "_is_large_model"):
                ...
                model_name = (
                    getattr(self.llm_strategy, "model_name", "") or config.model.name
                )
            ...
            if not torch.cuda.is_available():
                logger.error(
                    "GPU required for large models (30B+), but CUDA is not available"
                )
                return False

            # Check if device is set to CUDA
            device = (
                getattr(self.llm_strategy, "device", self.device) or config.model.device
            )
            if not device.startswith("cuda"):
                logger.error(
                    f"GPU required for large models (30B+), but device is set to '{device}'"
                )
                return False
            ...
            try:
                ...
                if test_result != 0:
                    logger.error(
                        "GPU tensor test failed. GPU required for large models."
                    )
                    return False
                ...
                if (
                    ...
                    logger.error(
                        f"GPU not compatible: {e}. GPU required for large models."
                    )
                    return False
                raise
        except Exception as e:
    ...
    async def recover_from_error(self, error_type: str = "unknown"):
        ...
        if error_type == "out_of_memory":
            logger.info(
                "OOM recovery: Cleared caches. Next request should use lower max_tokens."
            )
...
async def serve():
    ...
    if RATE_LIMIT_AVAILABLE:
        ...
        logger.info(
            "Rate limiting enabled for Inference API (in-memory, Redis not required)"
        )
