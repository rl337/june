...
class TestVoiceMessageFlow:
    ...
    async def test_stt_service_transcription(self, services_available):
        ...
        async with grpc.aio.insecure_channel(STT_ADDRESS) as channel:
            ...
            result = await client.recognize(wav_data, sample_rate=SAMPLE_RATE, encoding="wav", config=cfg)
...
class TestErrorScenarios:
    ...
    async def test_missing_audio_file(self, services_available):
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            logger.info(f"Missing audio file handled with status {response.status_code}")
            ...
            async with grpc.aio.insecure_channel(invalid_address) as channel:
                ...
                with pytest.raises(Exception):
                    await asyncio.wait_for(
                        client.recognize(b"test", sample_rate=16000, encoding="wav", config=cfg),
                        timeout=5.0
                    )
        except Exception as e:
            # Expected to fail
...
class TestConcurrentRequests:
    ...
    async def test_concurrent_stt_requests(self, services_available):
        ...
        successful = sum(1 for _, transcript in results if not transcript.startswith("Error"))
    ...
    async def test_concurrent_gateway_requests(self, services_available):
        ...
        num_requests = 5
        audio_data = generate_test_audio("Gateway concurrent test", duration_seconds=0.5)
        wav_data = create_wav_file(audio_data)
        ...
        logger.info(f"Concurrent Gateway requests: {successful}/{num_requests} successful")
        assert successful > 0  # At least some should succeed
    ...
    async def test_concurrent_full_round_trip_requests(self, services_available):
        ...
        # Verify all requests completed
        assert len(results) == num_requests
        successful = sum(1 for _, result in results if not result.startswith("Error") and not result.startswith("Status"))
        logger.info(f"Concurrent full round-trip requests: {successful}/{num_requests} successful")
        assert successful > 0  # At least some should succeed
