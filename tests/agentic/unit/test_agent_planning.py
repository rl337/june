...
class TestAgentPlanning:
    ...
    def test_planning_creates_execution_order(self):
        ...
        subtasks = [
            ...
            {"title": "Create API endpoint", "dependencies": ["Create database schema"]},
            {"title": "Add frontend component", "dependencies": ["Create API endpoint"]}
        ]
        ...
        frontend_idx = next(i for i, st in enumerate(order) if "frontend" in st["title"])
    ...
    def _analyze_requirements(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "keywords": keywords,
            "estimated_steps": ["Setup JWT library", "Create auth endpoint", "Test authentication"]
        }
    ...
    def _order_subtasks(self, subtasks: list) -> list:
        ...
        while remaining:
            for st in remaining[:]:
                ...
                if not deps or all(dep["title"] in [s["title"] for s in ordered] for dep in deps):
                    ordered.append(st)
                    remaining.remove(st)
                    break
...
class TestTaskDecomposition:
    ...
    def _decompose_with_relationships(self, task: Dict[str, Any]) -> list:
        ...
        return [
            {"title": "Setup payment API", "parent_id": task.get("id"), "type": "subtask"},
