...
class MockExecutionEnvironment:
    ...
    def _mock_python(self, args: List[str], cwd: Optional[str]) -> ExecutionResult:
        ...
        return ExecutionResult(
            success=False,
            stderr=f"File not found: {script}",
            returncode=1
        )
    ...
    def _mock_git(self, args: List[str], cwd: Optional[str]) -> ExecutionResult:
        """Mock git command."""
        if not args:
            return ExecutionResult(success=False, stderr="git: missing command", returncode=1)
    ...
    def _mock_cat(self, args: List[str], cwd: Optional[str]) -> ExecutionResult:
        """Mock cat command."""
        if not args:
            return ExecutionResult(success=False, stderr="cat: missing file operand", returncode=1)
        
        filename = args[0]
        if filename in self.files:
            return ExecutionResult(
                success=True,
                stdout=self.files[filename],
                returncode=0
            )
        else:
            return ExecutionResult(
