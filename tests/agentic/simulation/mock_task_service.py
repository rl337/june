...
class MockTaskService:
    ...
    def create_project(
        self, name: str, description: str, origin_url: str, local_path: str
    ) -> Project:
        """
    ...
    def list_available_tasks(
        self, agent_type: str, project_id: Optional[int] = None, limit: int = 10
    ) -> List[Task]:
        """
    ...
    def reserve_task(self, task_id: int, agent_id: str) -> Dict[str, Any]:
        ...
        if task.task_status != TaskStatus.AVAILABLE:
            raise ValueError(
                f"Task {task_id} is not available (status: {task.task_status})"
            )
    ...
    def add_task_update(
        ...
        return {"success": True, "update_id": update_id, "task_id": task_id}

    def complete_task(
        self, task_id: int, agent_id: str, notes: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        Complete a task.
        ...
        if task.assigned_agent != agent_id:
            raise ValueError(
                f"Task {task_id} is assigned to {task.assigned_agent}, not {agent_id}"
            )
        ...
        self.complete_calls.append(
            {"task_id": task_id, "agent_id": agent_id, "notes": notes}
        )
