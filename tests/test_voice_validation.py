...
INFERENCE_ADDRESS = os.getenv("INFERENCE_API_URL", "localhost:50051").replace(
    "grpc://", ""
)
...
class VoiceMessageValidationSuite:
    ...
    def __init__(self):
        self.results_dir = (
            Path(
                os.getenv("JUNE_TEST_DATA_DIR", os.path.expanduser("~/june_test_data"))
            )
            / "validation_tests"
        )
        ...
        try:
            ...
            async with grpc.aio.insecure_channel(STT_ADDRESS) as channel:
                ...
                result = await client.recognize(
                    audio_data, sample_rate=sample_rate, encoding="wav", config=cfg
                )
                latency = time.time() - start_time
    ...
    async def full_round_trip(
        ...
        transcript, stt_latency, confidence = await self.transcribe_audio(
    ...
    async def test_round_trip_accuracy(self, test_cases: List[str]) -> Dict[str, Any]:
        ...
        if wers:
            ...
            results["summary_metrics"]["average_word_accuracy"] = sum(
                word_accuracies
            ) / len(word_accuracies)
            results["summary_metrics"]["exact_match_rate"] = exact_matches / len(
                test_cases
            )
            results["summary_metrics"]["average_tts_latency"] = sum(
                tts_latencies
            ) / len(tts_latencies)
            results["summary_metrics"]["average_stt_latency"] = sum(
                stt_latencies
            ) / len(stt_latencies)
            results["summary_metrics"]["average_total_latency"] = sum(
                total_latencies
            ) / len(total_latencies)
    ...
    async def test_audio_quality_variations(self, base_text: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"Testing audio quality variations with text: '{base_text[:50]}...'"
        )
    ...
    async def test_audio_length_variations(
        self, duration_category: str, sample_texts: List[str]
    ) -> Dict[str, Any]:
        """Test with various audio lengths."""
        logger.info(
            f"Testing {duration_category} audio messages ({len(sample_texts)} tests)"
        )
        ...
        for i, text in enumerate(sample_texts):
            logger.info(
                f"Test {i+1}/{len(sample_texts)}: {duration_category} message ({len(text)} chars)"
            )
        ...
        if successful:
            ...
            avg_latency = sum(r["latencies"]["total"] for r in successful) / len(
                successful
            )
    ...
    async def test_response_quality(self, test_cases: List[str]) -> Dict[str, Any]:
        ...
        if wers:
            results["summary"] = {
                ...
                "wer_pass_rate": sum(
                    1 for w in wers if w <= results["quality_metrics"]["wer_threshold"]
                )
                / len(wers),
                "cer_pass_rate": sum(
                    1 for c in cers if c <= results["quality_metrics"]["cer_threshold"]
                )
                / len(cers),
                "word_accuracy_pass_rate": sum(
                    1
                    for a in word_accuracies
                    if a >= results["quality_metrics"]["word_accuracy_threshold"]
                )
                / len(word_accuracies),
            }
        ...
        if tts_latencies:
            results["summary"] = {
                "tts": {
                    ...
                    "pass_rate": sum(
                        1
                        for l in tts_latencies
                        if l <= results["latency_requirements"]["tts_max_seconds"]
                    )
                    / len(tts_latencies),
                },
                "stt": {
                    ...
                    "pass_rate": sum(
                        1
                        for l in stt_latencies
                        if l <= results["latency_requirements"]["stt_max_seconds"]
                    )
                    / len(stt_latencies),
                },
                "total": {
                    ...
                    "pass_rate": sum(
                        1
                        for l in total_latencies
                        if l <= results["latency_requirements"]["total_max_seconds"]
                    )
                    / len(total_latencies),
                },
            }
...
async def test_round_trip_accuracy(validation_suite):
    ...
    assert (
        results["summary_metrics"]["average_wer"] < 0.20
    ), "WER should be less than 20%"
    assert (
        results["summary_metrics"]["average_word_accuracy"] > 0.80
    ), "Word accuracy should be greater than 80%"
...
async def test_response_quality(validation_suite):
    ...
    ), "Average WER should be less than 15%"
    assert (
        results["summary"]["average_word_accuracy"] > 0.85
    ), "Average word accuracy should be greater than 85%"
...
async def test_latency(validation_suite):
    ...
    # Assertions
    assert (
        results["summary"]["tts"]["average"] < 5.0
    ), "Average TTS latency should be less than 5 seconds"
    assert (
        results["summary"]["stt"]["average"] < 10.0
    ), "Average STT latency should be less than 10 seconds"
    assert (
        results["summary"]["total"]["average"] < 15.0
    ), "Average total latency should be less than 15 seconds"
...
async def test_audio_length_short(validation_suite):
    ...
    assert len(results["test_results"]) == len(
        test_cases
    ), "Should test all short messages"
...
async def test_audio_length_medium(validation_suite):
    ...
    assert len(results["test_results"]) == len(
        test_cases
    ), "Should test all medium messages"
