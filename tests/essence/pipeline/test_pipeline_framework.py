...
class MockSTTService:
    ...
    async def recognize_stream(
        self, audio_chunks, sample_rate: int = 16000, encoding: str = "wav", config=None
    ):
...
class MockTTSService:
    ...
    async def synthesize(
        self, text: str, voice_id: str = "default", language: str = "en", config=None
    ) -> bytes:
        """Mock TTS synthesis."""
        self.call_count += 1
        if self.responses:
...
class PipelineTestFramework:
    ...
    async def run_pipeline(
        ...
        if self.use_real_services and check_services:
            ...
            if not (stt_available and tts_available and llm_available):
                ...
                self.metrics.warnings.append(
                    f"STT: {stt_available}, TTS: {tts_available}, LLM: {llm_available}"
                )
                return self.metrics
        ...
        try:
            ...
            if self.use_real_services:
                ...
                tts_audio = await self._real_tts_synthesize(
                    llm_response, language or "en"
                )
            else:
                # Use mock TTS
                tts_audio = await self.mock_tts.synthesize(
                    llm_response, language=language or "en"
                )
            ...
            self.metrics.errors.append(
                f"Missing dependencies for real services: {str(e)}"
            )
            self.metrics.warnings.append(
                "Install grpc and june_grpc_api packages to use real services"
            )
            self.metrics.total_duration = time.time() - start_time
            return self.metrics
    ...
    async def check_service_available(self, address: str, service_type: str) -> bool:
        ...
        try:
            ...
            if isinstance(grpc, MagicMock):
                logger.warning(
                    f"grpc module is mocked - cannot check {service_type} service"
                )
                return False
        except ImportError:
            logger.warning(
                f"grpc module not available - cannot check {service_type} service"
            )
            return False
        ...
        try:
            async with grpc.aio.insecure_channel(address) as channel:
                ...
                try:
                    ...
                    logger.warning(
                        f"✗ {service_type} service not reachable at {address}: {e}"
                    )
                    return False
        except Exception as e:
    ...
    async def _real_stt_recognize(
        ...
        try:
            ...
            if isinstance(asr_shim, MagicMock):
                raise ImportError(
                    "june_grpc_api module is mocked - cannot use real STT service"
                )
        except ImportError as e:
            raise ImportError(
                f"Required modules not available for real STT service: {e}"
            )
        ...
        try:
            async with grpc.aio.insecure_channel(stt_address) as channel:
                ...
                cfg = asr_shim.RecognitionConfig(
                    language=language, interim_results=False
                )
    ...
    async def _real_llm_chat(self, messages: List[Dict[str, str]]) -> str:
        ...
        try:
            ...
            if isinstance(llm_shim, MagicMock):
                raise ImportError(
                    "june_grpc_api module is mocked - cannot use real LLM service"
                )
        except ImportError as e:
            raise ImportError(
                f"Required modules not available for real LLM service: {e}"
            )
    ...
    async def _real_tts_synthesize(self, text: str, language: str = "en") -> bytes:
        ...
        try:
            ...
            if isinstance(tts_shim, MagicMock):
                raise ImportError(
                    "june_grpc_api module is mocked - cannot use real TTS service"
                )
        except ImportError as e:
            raise ImportError(
                f"Required modules not available for real TTS service: {e}"
            )
        ...
        try:
            async with grpc.aio.insecure_channel(tts_address) as channel:
                ...
                audio = await client.synthesize(
                    text=text, voice_id="default", language=language, config=cfg
                )
                return audio
        except Exception as e:
    ...
    def assert_performance(
        self, metrics: PipelineMetrics, max_total_duration: float = 30.0
    ):
        """Assert that pipeline meets performance requirements."""
        assert (
            metrics.total_duration < max_total_duration
        ), f"Pipeline took {metrics.total_duration}s, expected < {max_total_duration}s"

    def generate_test_audio(
        self, text: str = "Hello world", duration_seconds: float = 1.0
    ) -> bytes:
