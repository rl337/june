...
from essence.chat.platform_validators import TelegramValidator, DiscordValidator, get_validator
...
class TestTelegramValidator:
    ...
    @pytest.mark.parametrize("markdown,should_be_valid,expected_errors", [
        # Valid cases
        ("Hello world", True, []),
        ("*bold* text", True, []),
        ("_italic_ text", True, []),
        ...
        ("```python\ncode with *asterisk*\n```", True, []),  # Code blocks should escape content
    ...
    def test_validation(self, markdown, should_be_valid, expected_errors):
        ...
        assert is_valid == should_be_valid, f"Validation failed for: {markdown}\nErrors: {errors}"
        
        if expected_errors:
            # Check that at least one expected error is present
            error_text = " ".join(errors).lower()
            found_expected = any(
                expected.lower() in error_text
                for expected in expected_errors
            )
            assert found_expected, f"Expected one of {expected_errors} in errors: {errors}"
...
class TestDiscordValidator:
    ...
    @pytest.mark.parametrize("markdown,should_be_valid,expected_errors", [
        ...
        # Unsupported features
        ("# Heading", False, ["Heading syntax"]),
        ("| Table |\n| --- |\n| Cell |", False, ["Table syntax"]),  # Outside code block
        ("```\n| Table |\n| --- |\n| Cell |\n```", True, []),  # Inside code block is OK
