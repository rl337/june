...
from essence.chat.platform_translators import TelegramTranslator, DiscordTranslator, get_translator
...
class TestTelegramTranslator:
    ...
    def test_table_conversion(self):
        ...
        widget = TableWidget(
            rows=[
                TableRow(cells=[TableCell(text="Header 1"), TableCell(text="Header 2")], is_header=True),
                TableRow(cells=[TableCell(text="Cell 1"), TableCell(text="Cell 2")], is_header=False)
            ]
        )
        rendered = self.translator.render_widget(widget)
    ...
    def test_unbalanced_markdown_handling(self):
        ...
        if not is_valid:
            ...
            assert "\\*" in rendered or rendered.count("*") % 2 == 0, \
    ...
    def test_unbalanced_markdown_escaping(self, text):
        ...
        if not is_valid:
            ...
            assert has_escaped or rendered.count("*") % 2 == 0, \
                f"Translator should escape unbalanced markdown. Rendered: {rendered}, Errors: {errors}"
