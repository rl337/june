...
TEST_CASES = [
    ...
    (
        ...
        [
            (
                '{"type":"result","subtype":"success","result":"' + "x" * 5000 + '"}',
                False,
            ),  # Very long result
            ("", True),
        ],
...
@pytest.mark.parametrize(
    "name,input_lines,expected_outputs,expected_final_state", TEST_CASES
)
def test_json_accumulation_logic(
    name, input_lines, expected_outputs, expected_final_state
):
    """Test JSON accumulation logic with various input scenarios."""
    # Mock file system checks for agent script
    with patch("essence.chat.agent.response.os.path.exists") as mock_exists, patch(
        "essence.chat.agent.response.os.access"
    ) as mock_access, patch("essence.chat.agent.response.time.time") as mock_time:
        ...
        time_values = [0.0, 0.1, 0.2, 2.1, 2.2] + [
            2.3 + i * 0.1 for i in range(100)
        ]  # Start at 0, then jump past max_wait_for_first_message
...
def test_simple_complete_json():
    ...
    with patch("essence.chat.agent.response.os.path.exists") as mock_exists, patch(
        ...
        with patch("essence.chat.agent.response.streaming_popen_generator") as mock_gen:
            ...
            assert (
                len(outputs) > 0
            ), f"Should have outputs (even if error), got: {outputs}"
...
def test_json_accumulation_debug():
    ...
    with patch("essence.chat.agent.response.os.path.exists") as mock_exists, patch(
        ...
        with patch("essence.chat.agent.response.streaming_popen_generator") as mock_gen:
            ...
            assert (
                len(outputs) > 0
            ), f"Should have at least one output. Got {len(outputs)} outputs: {outputs}"

            # Last output should be final
            if len(outputs) > 0:
                last_msg, last_final, last_type = outputs[-1]
                print(
                    f"Last output: msg='{last_msg[:50]}...', final={last_final}, type={last_type}"
                )
