...
class TestReasoningPerformance:
    ...
    def test_cache_performance(self):
        ...
        mock_llm = MockLLMClientWithTiming(base_delay=0.05)  # Slower LLM to see cache benefit
        ...
        initial_llm_calls = mock_llm.think_calls + mock_llm.plan_calls + mock_llm.reflect_calls
        ...
        assert cache_stats['hits'] > 0 or cache_stats['misses'] > 0  # Cache is being used
    ...
    def test_concurrent_requests(self):
        ...
        def run_reasoning(request_id: int):
            ...
            try:
                ...
                context = ConversationContext(user_id=f"test_{request_id}", chat_id=f"test_{request_id}")
                start = time.time()
                result = reasoner.reason(
...
@pytest.mark.skip(reason="Requires real TensorRT-LLM service - run manually when service is available")
