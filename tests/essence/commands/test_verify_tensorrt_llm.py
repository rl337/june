...
class TestVerifyTensorRTLLMCommand:
    ...
    def test_add_args(self):
        ...
        args = parser.parse_args([
            "--tensorrt-llm-url", "http://test:8002",
            "--grpc-port", "9000",
            "--grpc-host", "test-host",
    ...
    @patch('sys.exit')
    def test_run_all_checks_pass(self, mock_exit, mock_gpu, mock_repo, mock_grpc, mock_container):
    ...
    @patch('essence.commands.verify_tensorrt_llm.check_gpu_availability')
    @patch('sys.exit')
    def test_run_critical_checks_fail(self, mock_exit, mock_gpu, mock_repo, mock_grpc, mock_container):
        """Test running verification when critical checks fail."""
        mock_container.return_value = (False, "Connection failed", {'accessible': False})
    ...
    @patch('sys.exit')
    @patch('json.dumps')
    def test_run_json_output(self, mock_json_dumps, mock_exit, mock_gpu, mock_repo, mock_grpc, mock_container):
