...
class TestCheckGRPCProtocolCompatibility:
    ...
    def test_protocol_compatibility_import_error(self, mock_grpc):
        ...
        with patch('essence.commands.verify_nim.GRPC_AVAILABLE', True):
            with patch('builtins.__import__', side_effect=ImportError("No module named june_grpc_api")):
                success, msg, details = check_grpc_protocol_compatibility("test:8001")
...
class TestVerifyNIMCommand:
    ...
    @patch('essence.commands.verify_nim.check_gpu_availability')
    @patch('sys.exit')
    def test_run_with_protocol_check(self, mock_exit, mock_gpu, mock_protocol, mock_grpc, mock_http):
        """Test run method with protocol check enabled."""
        mock_http.return_value = (True, "HTTP OK", {'accessible': True})
        mock_grpc.return_value = (True, "gRPC OK", {'connectable': True})
        mock_protocol.return_value = (True, "Protocol OK", {'protocol_compatible': True, 'model_name': 'qwen3'})
        mock_gpu.return_value = (True, "GPU OK", {'gpu_available': True})
    ...
    def test_run_json_output(self, mock_print, mock_exit, mock_gpu, mock_grpc, mock_http):
        ...
        json_calls = [call for call in mock_print.call_args_list if 'json.dumps' in str(call) or any('{' in str(arg) for arg in call[0])]
        assert len(json_calls) > 0 or any('json' in str(call).lower() for call in mock_print.call_args_list)
        mock_exit.assert_called_once_with(0)
