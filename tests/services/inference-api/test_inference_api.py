...
def mock_embedding_model():
    ...
    mock_model = MagicMock()
    mock_model.return_value = MagicMock(last_hidden_state=torch.randn(2, 10, 384))
    return mock_model
...
class TestGeneration:
    ...
    async def test_generate_stream_success(self, service_instance):
        ...
        request = GenerationRequest(
            prompt="Hello world",
            params=GenerationParameters(max_tokens=50, temperature=0.7, top_p=0.9),
    ...
    async def test_generate_stream_is_final_flag(self, service_instance):
        ...
        for i, chunk in enumerate(chunks):
            if i == len(chunks) - 1:
                assert (
                    chunk.is_final is True
                ), f"Last chunk (index {i}) should have is_final=True"
            else:
                assert (
                    chunk.is_final is False
                ), f"Non-final chunk (index {i}) should have is_final=False"
...
class TestChat:
    ...
    async def test_chat_stream_long_conversation(self, service_instance):
        ...
        messages = [
            ...
            ChatMessage(
                role="user", content="Tell me a long story about " + "adventure " * 20
            ),
    ...
    async def test_chat_stream_is_final_flag(self, service_instance):
        ...
        messages = [ChatMessage(role="user", content="Generate a detailed response")]
...
class TestEmbeddings:
    ...
    async def test_embed_error(self, service_instance):
        ...
        request = EmbeddingRequest(texts=["Hello world"], model="test-model")
...
class TestHealthCheck:
    ...
    async def test_health_check_healthy(self, service_instance):
        ...
        with patch.object(
            ...
            service_instance, "_check_database_health", return_value=True
        ), patch.object(
            service_instance, "_check_minio_health", return_value=True
        ), patch.object(
            service_instance, "_check_nats_health", return_value=True
        ):
...
class TestModelLoading:
    ...
    async def test_load_models_success(self, service_instance):
        ...
        with patch("main.Qwen3LlmStrategy") as mock_strategy_class, patch(
            "main.AutoModelForCausalLM"
        ) as mock_model_class, patch("main.AutoTokenizer") as mock_tokenizer_class:
            mock_strategy = MagicMock()
...
class TestServiceConnections:
    ...
    async def test_connect_services_success(self, service_instance):
        ...
        with patch("main.create_async_engine") as mock_engine, patch(
            "main.Minio"
        ) as mock_minio, patch("main.nats.connect") as mock_nats:
...
class TestHealthChecks:
    ...
    async def test_check_minio_health_unhealthy(self, service_instance):
        ...
        service_instance.minio_client.bucket_exists.side_effect = Exception(
            "MinIO error"
        )
...
class TestStreamingErrorHandling:
    ...
    async def test_chat_stream_error_during_streaming(self, service_instance):
        ...
        service_instance.llm_strategy.infer.side_effect = Exception(
...
class TestStreamingPerformance:
    ...
    async def test_chat_stream_throughput(self, service_instance):
        ...
        messages = [ChatMessage(role="user", content="Generate a longer response")]
