...
stt_service_dir = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../../services/stt")
)
...
class TestSTTMetricsStorage:
    ...
    def test_record_transcription_with_metadata(self):
        ...
        metadata = {"telegram_file_id": "test_file_123", "telegram_duration": 5}
    ...
    def test_get_metrics_summary_problematic_formats(self):
        ...
        unknown_format = next(
            (f for f in problematic if f["format"] == "unknown"), None
        )
        assert unknown_format is not None
        assert (
            unknown_format["avg_confidence"] < 0.7 or unknown_format["error_rate"] > 10
        )
