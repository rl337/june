...
class TestRecognitionStream:
    ...
    async def test_recognize_stream_error(self, service_instance):
        ...
        # Mock Whisper error
        service_instance.whisper_model.transcribe.side_effect = Exception(
            "Whisper error"
        )
...
class TestRecognitionOneShot:
    ...
    async def test_recognize_success(self, service_instance, sample_audio_data):
        ...
        assert (
            response.results[0].detected_language == "en"
...
class TestModelLoading:
    ...
    async def test_load_models_success(self, service_instance):
        ...
        with patch("main.whisper.load_model") as mock_load_model, patch(
            "main.webrtcvad.Vad"
        ) as mock_vad_class:
...
class TestSTTIntegration:
    ...
    @pytest.mark.asyncio
    async def test_language_detection_auto(
        self, service_instance, sample_audio_data, mock_whisper_model
    ):
        ...
        assert (
            "language" not in call_args.kwargs
            or call_args.kwargs.get("language") is None
        )

    @pytest.mark.asyncio
    async def test_language_detection_explicit(
        self, service_instance, sample_audio_data, mock_whisper_model
    ):
        ...
        request = RecognitionRequest(
            ...
            encoding="pcm",
            config=RecognitionConfig(
                language="en", interim_results=False  # Explicitly provided
            ),
        )
    ...
    @pytest.mark.asyncio
    async def test_language_detection_none(
        self, service_instance, sample_audio_data, mock_whisper_model
    ):
