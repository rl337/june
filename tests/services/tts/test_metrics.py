...
TTS_REQUESTS_TOTAL = Counter('tts_requests_total', 'Total TTS requests', ['status'], registry=TEST_REGISTRY)
TTS_SYNTHESIS_TIME = Histogram('tts_synthesis_time_seconds', 'TTS synthesis time', registry=TEST_REGISTRY)
TTS_AUDIO_DURATION = Histogram('tts_audio_duration_seconds', 'Generated audio duration', registry=TEST_REGISTRY)
TTS_ERRORS_TOTAL = Counter('tts_errors_total', 'Total errors', ['error_type'], registry=TEST_REGISTRY)
ACTIVE_CONNECTIONS = Gauge('tts_active_connections', 'Active gRPC connections', registry=TEST_REGISTRY)
...
class TestTTSMetrics:
    ...
    def test_metrics_with_different_status_labels(self):
        ...
        assert 'status="success"' in metrics_output or "status='success'" in metrics_output
        assert 'status="error"' in metrics_output or "status='error'" in metrics_output
        assert 'status="timeout"' in metrics_output or "status='timeout'" in metrics_output
    ...
    def test_histogram_metrics_aggregation(self):
        ...
        assert '_bucket' in metrics_output or 'count' in metrics_output.lower() or 'sum' in metrics_output.lower()
