...
def test_score_high_quality_audio(sample_wav_high_quality):
    ...
    ), f"Overall score too low: {result['overall_score']}"
    assert (
        result["volume_score"] >= 20
    ), f"Volume score too low: {result['volume_score']}"
    assert (
        result["clarity_score"] >= 20
    ), f"Clarity score too low: {result['clarity_score']}"
...
def test_score_low_volume_audio(sample_wav_low_volume):
    ...
    ), f"Volume score too high for low-volume audio: {result['volume_score']}"

    # Feedback should mention volume issues
    assert (
        "volume" in result["feedback"].lower() or "quiet" in result["feedback"].lower()
    )

    # Suggestions should include volume-related advice
    suggestions_text = " ".join(result["suggestions"]).lower()
    assert (
        "microphone" in suggestions_text
        or "volume" in suggestions_text
        or "louder" in suggestions_text
    )
...
def test_score_noisy_audio(sample_wav_noisy):
    ...
    assert (
        0 <= result["noise_score"] <= 100
    ), f"Noise score out of range: {result['noise_score']}"
...
def test_score_clipped_audio(sample_wav_clipped):
    ...
    ), f"Clarity score too high for clipped audio: {result['clarity_score']}"
...
def test_auto_format_detection():
    ...
    try:
        subprocess.run(
            ["ffprobe", "-version"], capture_output=True, timeout=5, check=True
        )
    except (
        FileNotFoundError,
        subprocess.TimeoutExpired,
