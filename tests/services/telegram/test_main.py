...
)  # tests/services/telegram/ directory
...
mock_admin_auth.require_admin = (
    lambda func: func
)  # Decorator that returns function as-is
...
if "opentelemetry" not in sys.modules or not hasattr(
    sys.modules["opentelemetry"], "trace"
):
...
class TestTelegramBotServiceInitialization:
    ...
    def test_init_success(
        self, mock_app_class, mock_get_stt_address, mock_get_service_config
    ):
    ...
    @patch.dict(
        os.environ,
        {"TELEGRAM_BOT_TOKEN": "test-token", "STT_URL": "grpc://custom-stt:50052"},
    )
    def test_init_custom_service_urls(
        self, mock_app_class, mock_get_stt_address, mock_get_service_config
    ):
...
class TestVoiceMessageHandling:
    ...
    @pytest.mark.asyncio
    async def test_handle_voice_message_file_too_large(
    ...
    async def test_handle_voice_message_success(
        ...
        mock_telegram_file.download_as_bytearray.return_value = bytearray(
            sample_ogg_audio
        )
        ...
        with patch("grpc.aio.insecure_channel") as mock_channel, patch(
            "handlers.voice.enhance_audio_for_stt"
        ) as mock_enhance, patch(
            "handlers.voice.prepare_audio_for_stt"
        ) as mock_prepare, patch(
            "june_grpc_api.asr.RecognitionConfig", MagicMock
        ), patch(
            "handlers.voice.AudioSegment"
        ) as mock_audio_segment, patch(
            "handlers.voice.export_audio_to_ogg_optimized"
        ) as mock_export_ogg, patch(
            "handlers.voice.find_optimal_compression"
        ) as mock_find_compression:
            ...
            mock_audio_segment.from_file.return_value = (
                mock_audio  # For fallback format detection
            )
            ...
            mock_httpx_module.AsyncClient.return_value.__aenter__ = AsyncMock(
                return_value=mock_client
            )
            mock_httpx_module.AsyncClient.return_value.__aexit__ = AsyncMock(
                return_value=False
            )

            # Mock tempfile and file operations
            with patch(
                "handlers.voice.tempfile.NamedTemporaryFile"
            ) as mock_tempfile, patch("handlers.voice.os.unlink") as mock_unlink, patch(
    ...
    async def test_handle_voice_message_audio_validation_error(
        ...
        with patch(
            "handlers.voice.prepare_audio_for_stt",
            side_effect=AudioValidationError("Audio too long"),
        ):
            await handle_voice_message(
                mock_update,
                mock_context,
    ...
    async def test_handle_voice_message_stt_error(
        ...
        mock_prepare = patch(
            "handlers.voice.prepare_audio_for_stt", return_value=b"mock_prepared_audio"
        )

        # Mock STT to raise error
        mock_stt_client = MagicMock()
        mock_stt_client.recognize = AsyncMock(
            side_effect=Exception("STT service unavailable")
        )

        with patch("grpc.aio.insecure_channel") as mock_channel, patch(
            "june_grpc_api.asr.SpeechToTextClient", return_value=mock_stt_client
        ), mock_prepare:
            # Mock async context manager for channel
            mock_channel.return_value.__aenter__ = AsyncMock(return_value=MagicMock())
            mock_channel.return_value.__aexit__ = AsyncMock(return_value=False)
...
class TestErrorHandlingPaths:
    ...
    async def test_handle_voice_message_no_file_size(
        ...
        with patch("grpc.aio.insecure_channel") as mock_channel, patch(
            ...
            "june_grpc_api.shim.llm.LLMClient"
        ) as mock_llm_client_class, patch(
            "june_grpc_api.shim.tts.TextToSpeechClient"
        ) as mock_tts_client_class, patch(
            "handlers.voice.AudioSegment"
            ...
            mock_llm_client_class.return_value.__aenter__ = AsyncMock(
            ...
            mock_tts_client_class.return_value.__aenter__ = AsyncMock(
...
class TestAudioFormatConversionIntegration:
    ...
    async def test_audio_conversion_in_voice_processing(
        ...
        with patch("grpc.aio.insecure_channel") as mock_channel, patch(
            ...
            "handlers.voice.enhance_audio_for_stt", return_value=b"mock_prepared_audio"
        ) as mock_enhance, patch(
            "handlers.voice.prepare_audio_for_stt", return_value=b"mock_prepared_audio"
        ) as mock_prepare, patch(
            "june_grpc_api.shim.llm.LLMClient"
...
class TestServiceRun:
    ...
    def service(self):
        ...
        with patch("main.config") as mock_config, patch(
            "main.Application"
        ) as mock_app_class, patch.dict(
