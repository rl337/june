...
class TestTelegramMetrics:
    ...
    def test_http_requests_total_metric(self):
        ...
        HTTP_REQUESTS_TOTAL.labels(method='POST', endpoint='/agent/message', status_code='200').inc()
        ...
        assert 'status_code="200"' in metrics_output or "status_code='200'" in metrics_output
    
    def test_http_request_duration_metric(self):
        """Test that HTTP request duration is recorded."""
        # Record request duration
        HTTP_REQUEST_DURATION_SECONDS.labels(method='GET', endpoint='/health', status_code='200').observe(0.1)
        HTTP_REQUEST_DURATION_SECONDS.labels(method='GET', endpoint='/metrics', status_code='200').observe(0.05)
    ...
    def test_grpc_requests_total_metric(self):
        ...
        GRPC_REQUESTS_TOTAL.labels(service='stt', method='recognize_stream', status_code='200').inc()
        GRPC_REQUESTS_TOTAL.labels(service='llm', method='chat_stream', status_code='200').inc()
        GRPC_REQUESTS_TOTAL.labels(service='tts', method='synthesize', status_code='200').inc()
    ...
    def test_grpc_request_duration_metric(self):
        ...
        GRPC_REQUEST_DURATION_SECONDS.labels(service='stt', method='recognize_stream', status_code='200').observe(1.5)
        GRPC_REQUEST_DURATION_SECONDS.labels(service='llm', method='chat_stream', status_code='200').observe(3.2)
    ...
    def test_voice_messages_processed_metric(self):
        ...
        VOICE_MESSAGES_PROCESSED_TOTAL.labels(platform='telegram', status='success').inc()
        ...
        assert 'platform="telegram"' in metrics_output or "platform='telegram'" in metrics_output
        assert 'status="success"' in metrics_output or "status='success'" in metrics_output
    ...
    def test_voice_processing_duration_metric(self):
        ...
        VOICE_PROCESSING_DURATION_SECONDS.labels(platform='telegram', status='success').observe(5.5)
        VOICE_PROCESSING_DURATION_SECONDS.labels(platform='telegram', status='error').observe(2.0)
    ...
    def test_stt_transcription_duration_metric(self):
        ...
        STT_TRANSCRIPTION_DURATION_SECONDS.labels(platform='telegram', status='success').observe(2.0)
        STT_TRANSCRIPTION_DURATION_SECONDS.labels(platform='telegram', status='error').observe(0.5)
    ...
    def test_tts_synthesis_duration_metric(self):
        ...
        TTS_SYNTHESIS_DURATION_SECONDS.labels(platform='telegram', status='success').observe(1.5)
        TTS_SYNTHESIS_DURATION_SECONDS.labels(platform='telegram', status='error').observe(0.3)
    ...
    def test_llm_generation_duration_metric(self):
        ...
        LLM_GENERATION_DURATION_SECONDS.labels(platform='telegram', status='success').observe(3.0)
        LLM_GENERATION_DURATION_SECONDS.labels(platform='telegram', status='error').observe(1.0)
    ...
    def test_errors_total_metric(self):
        ...
        assert 'service="telegram"' in metrics_output or "service='telegram'" in metrics_output
        assert 'error_type="ValueError"' in metrics_output or "error_type='ValueError'" in metrics_output
        assert 'error_type="ConnectionError"' in metrics_output or "error_type='ConnectionError'" in metrics_output
    ...
    def test_metrics_with_different_labels(self):
        ...
        HTTP_REQUESTS_TOTAL.labels(method='GET', endpoint='/health', status_code='200').inc()
        HTTP_REQUESTS_TOTAL.labels(method='GET', endpoint='/health', status_code='503').inc()
        HTTP_REQUESTS_TOTAL.labels(method='POST', endpoint='/agent/message', status_code='200').inc()
        ...
        assert 'http_requests_total' in metrics_output
        assert 'status_code="200"' in metrics_output or "status_code='200'" in metrics_output
        assert 'status_code="503"' in metrics_output or "status_code='503'" in metrics_output
        assert 'method="GET"' in metrics_output or "method='GET'" in metrics_output
