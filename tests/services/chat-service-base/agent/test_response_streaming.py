...
class TestExtractHumanReadable:
    ...
    def test_extract_assistant_message_incremental_chunk_1(self):
        ...
        json_line = json.dumps(
            {
                "type": "assistant",
                "message": {"content": [{"type": "text", "text": "Hi. How can"}]},
            }
        )
    ...
    def test_extract_middle_chunk_problem(self):
        ...
        json_line = json.dumps(
            {
                "type": "assistant",
                "message": {"content": [{"type": "text", "text": "order or not al"}]},
            }
        )
        ...
        json_line = json.dumps(
            {
                "type": "assistant",
                "message": {
                    "content": [{"type": "text", "text": "Writing response to file..."}]
                },
            }
        )
...
class TestStreamAccumulation:
    ...
    def test_accumulation_with_incremental_chunks(self):
        ...
        with patch(
            "essence.chat.agent.response.streaming_popen_generator", mock_generator
        ):
        ...
        if len(message_lengths) > 1:
            ...
            assert all(
                msg_len >= message_lengths[0] for msg_len in message_lengths
    ...
    def test_accumulation_with_middle_chunk_problem(self):
        ...
        for msg in messages:
            if msg == "order or not al":
                ...
                pytest.fail(
                    "Should not yield standalone middle chunk 'order or not al'"
                )

        # The final message should contain the full text
        final_messages = [
            msg for msg, is_final, msg_type in results if msg and not is_final
        ]
        if final_messages:
            longest = max(final_messages, key=len)
            assert (
                "Hi. How can" in longest or "order or not" in longest
    ...
    def test_always_use_longest_message(self):
        ...
        json_lines = [
            ...
            json.dumps(
                {
                    ...
                    "message": {
                        "content": [
                            {
                                "type": "text",
                                "text": "This is a much longer message that contains Short",
                            }
                        ]
                    },
                }
            ...
            json.dumps(
                {
                    "type": "assistant",
                    "message": {"content": [{"type": "text", "text": "Medium length"}]},
                }
            ),
        ]
        ...
        if messages:
            ...
            assert (
                "This is a much longer message" in longest_yielded
                or len(longest_yielded) >= 40
            )
...
class TestMiddleChunkProblem:
    ...
    def test_accumulation_skips_middle_chunks(self):
            ...
            json.dumps(
                {
                    ...
                    "message": {
                        "content": [
                            {
                                "type": "text",
                                "text": "Hi. How can I help you? You can order or not all of the items.",
                            }
                        ]
                    },
                }
        ...
        assert (
            len(standalone_middle_chunks) == 0
    ...
    def test_partial_text_extraction(self):
        ...
        test_cases = [
            ...
            {"type": "assistant", "text": "order or not al"},
            # Case 2: Nested in result
            {"type": "result", "subtype": "success", "result": "order or not al"},
            # Case 3: In message field directly
            {"type": "assistant", "message": "order or not al"},
        ]
        ...
        for test_case in test_cases:
            ...
            if result:
                assert result == "order or not al" or len(result) > len(
                    "order or not al"
                )
...
class TestRealWorldScenario:
    ...
    def test_full_message_scenario(self):
        ...
        if len(messages) > 1:
            for i in range(1, len(messages)):
                assert (
                    len(messages[i]) >= len(messages[i - 1])
                    or messages[i - 1] in messages[i]
        ...
        final_non_empty = (
            [msg for msg, is_final, msg_type in results if msg][-1] if results else None
        )
    ...
    def test_reproduces_middle_chunk_issue(self):
            ...
            json.dumps(
                {
                    ...
                    "message": {
                        "content": [
                            {
                                "type": "text",
                                "text": "Hi. How can I help you? You can order or not all items.",
                            }
                        ]
                    },
                }
        ...
        if all_messages:
            ...
            assert (
                "Hi. How can" in longest or len(longest) > 20
