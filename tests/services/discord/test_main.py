...
)  # tests/services/discord/ directory
...
if "discord" in sys.modules:
    ...
    if hasattr(mod, "__file__") and mod.__file__:
        ...
        if _local_discord_dir and _local_discord_dir in mod.__file__:
            ...
            "tests/services/discord" in mod.__file__
            or "essence/services/discord" in mod.__file__
        ):
            del sys.modules["discord"]
...
class TestDiscordBotServiceInitialization:
    ...
    @patch.dict(
        os.environ, {"DISCORD_BOT_TOKEN": "test-token", "DISCORD_SERVICE_PORT": "9090"}
    )
    @patch("essence.services.discord.main.commands.Bot")
    @patch("essence.services.discord.main.FastAPI")
...
class TestDiscordMessageHandling:
    ...
    async def test_handle_message_success(
        ...
        mock_stream_agent.return_value = iter(
            [("Hello, user!", False, "assistant"), ("", True, None)]
        )

        # Mock MessageBuilder
        with patch(
            "essence.services.discord.main.MessageBuilder"
    ...
    async def test_handle_message_error(
        ...
        # Mock error handler
        with patch(
            "essence.chat.error_handler.render_error_for_platform"
    ...
    @patch("essence.services.discord.main.FastAPI")
    @patch("essence.services.discord.main.stream_agent_message")
    async def test_handle_message_no_response(
        self,
        mock_stream_agent,
        mock_fastapi_class,
        mock_bot_class,
        ...
        mock_stream_agent.return_value = iter([("", True, None)])
    ...
    @patch("essence.services.discord.main.commands.Bot")
    @patch("essence.services.discord.main.FastAPI")
    async def test_handle_message_bot_message_ignored(
        self, mock_fastapi_class, mock_bot_class
    ):
...
class TestDiscordHealthEndpoints:
    ...
    @patch("essence.services.discord.main.commands.Bot")
    @patch("essence.services.discord.main.FastAPI")
    async def test_health_check_unhealthy_missing_token(
        self, mock_fastapi_class, mock_bot_class
    ):
...
class TestDiscordErrorHandling:
    ...
    @patch("essence.services.discord.main.FastAPI")
    @patch("essence.services.discord.main.stream_agent_message")
    async def test_handle_message_stream_error(
        self,
        mock_stream_agent,
        mock_fastapi_class,
        mock_bot_class,
    ...
    @patch("essence.services.discord.main.commands.Bot")
    @patch("essence.services.discord.main.FastAPI")
    async def test_handle_message_send_error(
        self, mock_fastapi_class, mock_bot_class, mock_discord_message
    ):
...
class TestDiscordServiceRun:
    ...
    async def test_run_shutdown(
        self, mock_server_class, mock_fastapi_class, mock_bot_class
    ):
