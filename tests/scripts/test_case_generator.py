...
class TestCaseGenerator:
    ...
    def generate_short_cases(self, count: int = 300) -> List[TestCase]:
        ...
        for _ in range(remaining):
            if random.random() < 0.4:  # 40% single words
                ...
                cases.append(
                    TestCase(f"{word1} {word2} {word3}", "short", "three_words_random")
                )
    ...
    def generate_all(
        self, short_count: int = 300, medium_count: int = 400, long_count: int = 300
    ) -> List[TestCase]:
    ...
    def get_statistics(self, cases: List[TestCase]) -> dict:
        ...
        for case in cases:
            ...
            stats["by_subcategory"][case.subcategory] = (
                stats["by_subcategory"].get(case.subcategory, 0) + 1
            )
            stats["word_counts"].append(len(case.text.split()))

        stats["avg_words"] = (
            sum(stats["word_counts"]) / len(stats["word_counts"])
