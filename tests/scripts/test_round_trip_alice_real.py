...
try:
    ...
    logger.error(
        "Make sure proto Python files are generated: python3 -m grpc_tools.protoc --python_out=proto --grpc_python_out=proto --proto_path=proto proto/*.proto"
    )
    sys.exit(1)
...
class RealRoundTripTester:
    ...
    def __init__(
        self, tts_address: str = "localhost:50053", stt_address: str = "localhost:50052"
    ):
        ...
        try:
            async with grpc.aio.insecure_channel(self.tts_address) as channel:
                ...
                config = SynthesisConfig(
                    sample_rate=self.sample_rate, speed=1.0, pitch=0.0
                )

                request = SynthesisRequest(
                    text=text, config=config, voice_id="default", language="en"
                )
                ...
                if response.audio_data:
                    logger.debug(
                        f"Received {len(response.audio_data)} bytes of audio from TTS"
                    )
        ...
        try:
            async with grpc.aio.insecure_channel(self.stt_address) as channel:
                ...
                if response.results and len(response.results) > 0:
                    ...
                    logger.debug(
                        f"STT recognized: '{transcript}' (confidence: {confidence:.2f})"
                    )
                    return transcript
                else:
    ...
    def calculate_metrics(self, original: str, transcribed: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(original_words) > 0:
                ...
                matching_words = sum(
                    1 for word in original_words if word in transcribed_words
                )
    ...
    async def run_round_trip_test(
        self, passages: List[Dict[str, Any]], max_tests: Optional[int] = None
    ) -> Dict[str, Any]:
        ...
        if not await self.test_connectivity():
            logger.error(
                "Services are not available. Please ensure STT and TTS services are running in Docker."
            )
            return {
        ...
        if max_tests:
            ...
            logger.info(
                f"Testing {len(test_passages)} passages (limited to {max_tests})"
            )
        else:
        ...
        results = {
            "test_timestamp": datetime.now().isoformat(),
            "service_addresses": {"tts": self.tts_address, "stt": self.stt_address},
        ...
        for i, passage in enumerate(test_passages):
            ...
            try:
                ...
                if not audio_data or len(audio_data) == 0:
                    logger.error(
                        f"  ❌ Failed to generate audio for passage {passage_id}"
                    )
                ...
                if not transcribed_text:
                    logger.error(
                        f"  ❌ Failed to transcribe audio for passage {passage_id}"
                    )
                    results["test_results"].append(
                        {
                            "passage_id": passage_id,
                ...
                logger.info(
                    f"  ✅ Transcribed: {transcribed_text[:100] if len(transcribed_text) > 100 else transcribed_text}..."
                )
                ...
                if metrics:
                    ...
                    logger.info(
                        f"  Word accuracy: {metrics['word_accuracy']:.2%}, Char accuracy: {metrics['char_accuracy']:.2%}"
                    )
                    ...
                    results["test_results"].append(
                        {
                            ...
                            "audio_duration_seconds": len(audio_data)
                            / (self.sample_rate * 2),
                        }
                    )
                ...
                results["test_results"].append(
                    {"passage_id": passage_id, "status": "failed", "error": str(e)}
                )
                results["failed_tests"] += 1
        ...
        if word_accuracies:
            ...
            results["summary_metrics"]["average_char_accuracy"] = sum(
                char_accuracies
            ) / len(char_accuracies)
...
def main():
    ...
    dataset_file = (
        Path(data_dir) / "datasets" / "alice_in_wonderland" / "alice_dataset.json"
    )
    ...
    logger.info(
        f"Success rate: {results['successful_tests']/results['total_tests']*100:.1f}%"
    )
    ...
    results_file = (
        ...
        / "datasets"
        / "alice_in_wonderland"
        / "real_round_trip_test_results.json"
    )
