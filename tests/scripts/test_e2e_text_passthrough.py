...
class EndToEndTextPassthroughTest:
    ...
    async def send_to_gateway(
        self, audio_data: bytes, session_id: str = "e2e_test"
    ) -> bytes:
        """Step 2: Send audio to Gateway with full_round_trip=true."""
        try:
            logger.info(
                f"Step 2: Gateway - Sending {len(audio_data)} bytes with full_round_trip=true"
            )
            ...
            async with httpx.AsyncClient(timeout=120.0) as client:
                ...
                if "audio_data" in result:
                    ...
                    logger.error(
                        f"  ❌ No audio_data in response. Available keys: {list(result.keys())}"
                    )
                    return b""
    ...
    async def test_text_passthrough(
        self, input_text: str, tolerance: str = "exact"
    ) -> bool:
        """
        ...
        try:
            ...
            if tolerance == "exact":
                ...
                logger.info(
                    f"  Comparison: {'✅ EXACT MATCH' if passed else '❌ MISMATCH'}"
                )
            elif tolerance == "contains":
                passed = (
                    input_normalized in output_normalized
                    or output_normalized in input_normalized
                )
                logger.info(
                    f"  Comparison: {'✅ OUTPUT CONTAINS INPUT' if passed else '❌ NO MATCH'}"
                )
            else:
...
async def main():
    ...
    tester = EndToEndTextPassthroughTest(
        tts_address=tts_address, stt_address=stt_address, gateway_url=gateway_url
    )
    ...
    test_cases = [
        ...
        (
            ...
            "contains",
        ),  # Longer text, may have slight variations
        ("Test", "exact"),
    ]
    ...
    for input_text, tolerance in test_cases:
        ...
        results.append({"input": input_text, "tolerance": tolerance, "passed": passed})
        logger.info("")
