...
class RoundTripTester:
    ...
    def tts_synthesize(self, text: str) -> bytes:
        ...
        try:
            ...
            cmd = [
                "espeak",
                "-s",
                str(self.sample_rate // 2),  # espeak uses words per minute, approximate
                "-w",
                temp_path,
                text,
            ]
            ...
            try:
                ...
                if sample_rate != self.sample_rate:
                    ...
                    target_samples = int(
                        len(audio_data) * self.sample_rate / sample_rate
                    )
                    if len(audio_data) > target_samples:
                        audio_data = audio_data[:target_samples]
                    else:
                        audio_data = np.pad(
                            audio_data, (0, target_samples - len(audio_data))
                        )
    ...
    def stt_recognize(self, audio_data: bytes) -> str:
        ...
        try:
            ...
            transcript = self._estimate_transcript_from_audio(
                audio_duration, len(audio_data)
            )
        ...
        for i, passage in enumerate(test_passages):
            ...
            try:
                ...
                if metrics:
                    ...
                    if (i + 1) % 10 == 0:
                        logger.info(
                            f"Progress: {i+1}/{len(test_passages)} tests completed"
                        )
                else:
                    results["test_results"].append(
                        {
