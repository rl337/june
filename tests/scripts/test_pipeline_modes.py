...
class ModeTester:
    ...
    def test_roundtrip_mode(self) -> Dict[str, Any]:
        ...
        logger.info(
            "Testing via Gateway: Text → TTS → Audio → Gateway → Audio → STT → Text"
        )
    ...
    def print_mode_summary(self):
        ...
        if config["test_mode"] == "mock":
            ...
            logger.info(
                "  - Two conversions: Input (Text→TTS→STT) and Output (Audio→STT)"
            )
        logger.info("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test June Agent pipeline in different modes"
    )
    parser.add_argument(
        "--mode",
        choices=["mock", "stt_tts_roundtrip", "both"],
        default=None,
