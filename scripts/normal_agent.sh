#!/bin/bash
# Normal Agent for June project - Standard task work
# Usage: normal_agent.sh [task_instruction]
...
export PROMPTS_DIR="${PROMPTS_DIR:-${AGENTICNESS_DIR}/prompts/normal}"
...
exec python3 -m agenticness.commands.execute_cursor_agent \
    ...
    ${TASK_INSTRUCTION:+--message "$TASK_INSTRUCTION"}
