...
Inspects a sandbox snapshot to analyze what the agent did to solve a benchmark task.
Shows file changes, command history, resource usage, efficiency metrics, etc.

Usage:
    python scripts/review_sandbox.py <sandbox_snapshot_dir>
    python scripts/review_sandbox.py <output_dir> <task_id>
"""
import argparse
import json
import sys
...
def print_efficiency_metrics(metadata: Dict[str, Any]) -> None:
    ...
    duration = metrics.get('end_time', 0) - metrics.get('start_time', 0) if metrics.get('end_time') else 0
...
def main():
    parser = argparse.ArgumentParser(description="Review sandbox snapshot")
    parser.add_argument(
        "path",
    ...
    snapshot_dir = Path(args.path)
    
    # If task_id is provided, treat path as output directory
    if args.task_id:
        found = find_sandbox_snapshot(snapshot_dir, args.task_id)
        if found:
            snapshot_dir = found
        else:
            print(f"Error: Sandbox snapshot not found for task {args.task_id} in {args.path}")
            sys.exit(1)
    
    if not snapshot_dir.exists():
        print(f"Error: Sandbox snapshot directory not found: {snapshot_dir}")
        sys.exit(1)
    ...
    if not metadata_file.exists():
        print(f"Error: Metadata file not found in {snapshot_dir}")
        print("Expected: sandbox_metadata.json")
        sys.exit(1)
